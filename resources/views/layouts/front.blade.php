...
<head>
    ...
    <meta name="_token" content="{!! csrf_token() !!}"/>
     <meta name="description" content="Embrace Your Love for Bowls: Exceptional UK Holidays Tailored for Club" />
    <meta name="robots" content="max-image-preview:large" />
    <meta name="google-site-verification" content="SzIDspjXPU2KWoa778YFs9d37Fs9sbDfu_ruICbMH9Q" />
    <meta property="og:locale" content="en_US" />
    <meta property="og:site_name" content="Bowling Tours - The UK&#039;s leading holiday operator" />
    <meta property="og:type" content="website" />
    <meta property="og:title" content="{{!empty($page_title)?$page_title:''}}" />
    <meta property="og:description" content="Embrace Your Love for Bowls: Exceptional UK Holidays Tailored for Club" />
    <meta property="og:url" content="{{URL('/')}}" />
    <meta name="twitter:card" content="summary_large_image" />
    <meta name="twitter:title" content="Bowling Tours - Unforgettable UK Lawn Bowls Holidays Await!" />
    <meta name="twitter:description" content="Embrace Your Love for Bowls: Exceptional UK Holidays Tailored for Club" />
             ...
             <div class="menuBox">
                ...
                <ul class="menu">
                    <li class="menuLi hasSubmenu">
                        <a class="menuLink" href="{{route('search.main')}}">UK Tours</a>
                    ...
                    <li class="menuLi">
                        <a class="menuLink" href="{{route('search.main')}}">Tours</a>
                    </li>
                    <li class="menuLi">
                        <a class="menuLink" href="{{route('about-us')}}">About Us</a>
                    </li>
                    <!-- <li class="menuLi">
                    ...
                    <li class="menuLi">
                        <a class="menuLink" href="{{route('contact-us')}}">Contact Us</a>
                    </li>
                    <!-- <li class="menuLi">
                ...
                <div class="header_right">
                     <ul class="menu" style="float:right;margin-top: 5px;">
                        <?php
                            ...
                            ?>
                       <li class="menuLi m-2">
                        <?php if(empty($bt_user)){ ?> 
                            <a target="_blank" href="{{getenv('IMAGE_URL')}}login/bt" style="background-color: #50AADB;padding: 12px 15px;border-radius: 5px;color: white;font-size:15px;">Log In</a>
                        <?php } else { ?> 
                            <a target="_blank" href="{{getenv('IMAGE_URL')}}alerts" style="background-color: #50AADB;padding: 12px 15px;border-radius: 5px;color: white;font-size:15px;">Account</a>
                        <?php } ?>
                            <a href="javascript:void();" style="background-color: #50AADB;padding: 12px 15px;border-radius: 5px;color: white;font-size:15px;" class="menuLink search_tour_nav " href=""><i class="fas fa-search"></i></a>
                            <?php if(!empty($bt_user)){ $url = 'cart?bt_tour='.$bt_user;?> 
                            <a href="{{url($url)}}" style="background-color: #50AADB;padding: 12px 15px;border-radius: 5px;color: white;font-size:15px;" class=" " href=""><i class="fas fa-shopping-cart" style="color: #ffffff; "></i></a>
                        <?php } ?>
                        </li>
                    </ul>
                    <input class="searchTerm" placeholder="What are you looking for?" type="text" value="" name="search_nav" id="search_nav" style="padding: 4px;margin-top: 6px;">
...
<script>
    ...
    $(document).ready(function(){
        $('.countryAreasBoxCls').hide();
         $('.countryAreasBoxCls').hide();
        /*alert($('.location_cls:checked').length)
        alert($('.location_cls').length)*/
        if ($('.location_cls:checked').length === $('.location_cls').length) {
          // All checkboxes are checked — so uncheck them all
          $('input.location_cls').prop('checked', true);
          $('input.country_areas_id').prop('checked', true);
        }
        else
        {
            $('#checkbox-select-all').prop('checked', false);
        }
         if ($('.ecatIdCls:checked').length === $('.ecatIdCls').length) {
          // All checkboxes are checked — so uncheck them all
          $('input.ecatIdCls').prop('checked', true);
        }
        else
        {
            $('#checkbox-select-all2').prop('checked', false);
        }
        $("#checkbox-select-all").bind('click', function (e) {
            if ($(this).prop('checked')) {
              ...
              {
                $('input.location_cls').prop('checked', false);
                $('input.country_areas_id').prop('checked', false);
              }
        });
        $("#checkbox-select-all2").bind('click', function (e) {
            if ($(this).prop('checked')) {
                $('input.ecatIdCls').prop('checked', true);
              } 
              else
              {
                $('input.ecatIdCls').prop('checked', false);
              }
        });
      
        $(".location_cls").bind('click', function (e) {
            ...
            else
            {
             ...
             var parentBox = $(this).closest('.parentBox');
              var childrenCheckboxes = parentBox.find('.country_areas_id');
              var id = $(this).val();
             if ($(this).prop('checked')) {
                
               //$('#checkbox-select-all').prop('checked', false);
               //$('.checkboxCLs-'+id).prop('checked', true);
               childrenCheckboxes.prop('checked', true);
               
              } 
              else
              {
                //$('.checkboxCLs-'+id).prop('checked', false);
                childrenCheckboxes.prop('checked', false);
              }
            
        })
        $(".country_areas_id").on('click', function () {
                if ($('#checkbox-select-all').prop('checked')) {
                    $('#checkbox-select-all').prop('checked', false);
                }
                var parentBox = $(this).closest('.parentBox');
                var childrenCheckboxes = parentBox.find('.country_areas_id');
                var parentCheckbox = parentBox.find('.location_cls');
         ...
         $(".ecatIdCls").bind('click', function (e) {
             if ($(this).prop('checked')) {
               $('#checkbox-select-all2').prop('checked', false);
              } 
               if ($('.ecatIdCls:checked').length === $('.ecatIdCls').length) {
                  // All checkboxes are checked — so uncheck them all
                  $('#checkbox-select-all2').prop('checked', true);
                }
                else
                {
                    $('#checkbox-select-all2').prop('checked', false);
...
<style type="text/css">
    ...
    margin: 0 !important;
    padding: 0 !important;
    background: url(/images/footer-bg.png) left top no-repeat !important;
    ...
    <div class="footer_container">
        <div class="footer_top">
            <div class="footer_left">
                <div class="footer_logo"><a href="#"><img src="{{ asset('images/homepage/footer-logo.png')}}"></a></div>
                <p>Sir Henry Darvill House, 8-10 William St, Windsor SL4 1BA</p>
                <h4>Follow Us
                    <a href="https://www.facebook.com/BowlingTours" target="_blank"><img src="{{ asset('images/homepage/facebook.png')}}"></a>
                    <!-- <a href="https://twitter.com/bowlingtours" target="_blank"><img src="{{ asset('images/homepage/twitter.png')}}"></a> -->
                    <!-- <a href="https://www.linkedin.com" target="_blank"><img src="{{ asset('images/homepage/in.png')}}"></a>
                    <a href="https://www.plus.google.com" target="_blank"><img src="{{ asset('images/homepage/google.png')}}"></a> -->
                    <a href="https://www.instagram.com/bowlingtours/" target="_blank"><img src="{{ asset('images/homepage/instagram.png')}}"></a></h4>
            </div>
            <div class="footer_right">
                <div class="footer_nav">
                    <h3>Useful Links</h3>
                    <ul><li id="menu-item-1664" class="menu-item menu-item-type-post_type menu-item-object-page menu-item-1664"><a href="{{route('about-us')}}">About BT</a></li>
<li id="menu-item-5050" class="menu-item menu-item-type-custom menu-item-object-custom menu-item-5050"><a href="/">Tours</a></li>
...
<li id="menu-item-553" class="menu-item menu-item-type-post_type menu-item-object-page menu-item-553"><a href="/terms-conditions/">Terms &amp; Conditions</a></li>
                ...
                <div class="footer_newsletter">
                    <h3>Contact Us</h3>
                    <p><a href="tel:01753836600"><img src="{{ asset('images/homepage/tel.png')}}"> 01753 836 600</a><br> <span><a href="mailto:groups@bowlingtours.co.uk"><img src="{{ asset('images/homepage/mail.png')}}"> groups@bowlingtours.co.uk</a></span></p>
                    <!-- <h4>Newsletter</h4> -->
        ...
        <!-- <div class="footer_bottom_left"><img src="{{ asset('images/homepage/footer-star.png')}}"></div> -->
...
<li id="menu-item-1483" class="menu-item menu-item-type-post_type menu-item-object-page menu-item-1483"><a href="https://bowlingtours-staging-co-uk.stackstaging.com{{route('about-us')}}">Bowling Tours 2022</a></li>
<li id="menu-item-63" class="menu-item menu-item-type-custom menu-item-object-custom menu-item-63"><a href="#">Designed &#038; Developed by <strong>CDA</strong></a></li>
         ...
         <div class="footer_bottom_right">
            <ul>
                ...
                <li id="menu-item-62">
                    <a href="{{route('about-us')}}" class="menu-image-title-after"><span class="menu-image-title"><strong>Bowling Tours 2024</strong></span></a>
                </li>
                <li id="menu-item-63">
...
<script>
    ...
    $("#search_nav").keyup(function(event) {
     ...
     $(document).on('click', '.search_tour_nav', function() {
        if($('#search_nav').val() != '')
        {
            window.location.href = "{{route('search.main')}}?&price_from=0&price_to=999999&search_text="+$('#search_nav').val();
        }
        else
        {
            $('#search_nav').show();
