    <div class="modal-header">
        <div class="row">
            <div class="col-sm-12">
                <h5 class="modal-title" id="exampleModalLongTitle"><b>Cancelled Dates - {{ $hotelList->name }}</b></h5>
            </div>
            <!-- <div class="col-sm-12">
...
DB::connection('mysql_veenus')->table('experiences_to_hotels_to_experience_dates')->selectRaw('ex.id,c.id as cart_id,c.signed_document,experiences_to_hotels_to_experience_dates.experience_dates_id,c.created_at as booked_date,c.cancel_reason,ex.experiences_id')
...
DB::connection('mysql_veenus')->table('experiences_to_hotels_to_experience_dates')->selectRaw('group_concat(DISTINCT e.tour_id) as tour_id,group_concat(DISTINCT e.id) as exp_id,group_concat(DISTINCT ed.id) as experience_dates_id,group_concat(DISTINCT ed.hotel_date_id) as hotel_date_id,group_concat(concat(ed.id,"_",ed.deleted_at)) as experience_deleted_at')->join('experience_dates as ed', 'ed.id', '=', 'experiences_to_hotels_to_experience_dates.experience_dates_id')->leftjoin('experiences as e', 'e.id', '=', 'ed.experiences_id')->where([
                            ...
                            <div class="row">
                                <div class="col" style="padding: 2px !important;margin-left: 18px;">
                                    <div class="form-group">
                                       ...
                                       <label class="al-label"><?=!empty($hotel_cart_list[0]->cancel_reason)?$hotel_cart_list[0]->cancel_reason:'N/A'?></label>
                                 ...
                                 <div class="col" style="padding: 2px !important;margin-left: 20px;">
                                    <div class="form-group">
                                        <!-- <a class="saveAllChangesBtn  yellowClrBtn marginTop15 clearBothCls float-left  " data-fancybox data-type="ajax" data-src="" href="/super-user/stocklist-view-dates/{{$hotel_book_date_id}}/{{$hotel_id}}/{{!empty($hotel_cart_list[0]->id)?$hotel_cart_list[0]->id:'0'}}?type=cancelled" style="font-size: 10px;">View</a> -->
                               ...
                               <!--  <div class="col" style="padding: 2px !important;margin-left: 18px;">
                                    <div class="form-group">
                                          <a class="yellowClrBtn marginTop15 clearBothCls float-left  edit-new notClickedCls <?=!empty($hotel_cart_list)?'':''?>" style="font-size: 12px;" target="_blank"  href="<?php echo 'stocklist-view-dates/'.$hotel_book_date_id.'/'.$hotel_id.'?type=view'; ?>" >View</a> 
