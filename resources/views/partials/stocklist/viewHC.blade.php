...
<?php 
            $cnts = 0110;
				...
				$is_hotel_assieged = 0;
				if(!empty($experienceDate->dates_rates_id))
				{
					$date_rate_id = $experienceDate->dates_rates_id;
					$hotel_cart_list =  
...
DB::connection('mysql_veenus')->table('cart_experiences as c')->selectRaw('c.id as cart_id,currency,c.cancel_reason')
					 ...
					 if(!empty($hotel_cart_list)) 
                        ...
                        {
                           $prefer_currency = $hotel->preferred_currency;
                        }
        ...
        <div class="tour_confirmation">
            ...
            <div class="tc_content" style="margin-top: 100px;">
                ...
                <div class="tc_boxes">
                    ...
                    <?php if(!empty($hotel_cart_list[0]->cancel_reason)){ ?> 
...
DB::connection('mysql_veenus')->table('hotel_dates_supplements')->where('hotel_dates_id', $hotel_book_date_id)->get()->toArray();
								...
								if(!empty($hotel_dates_supplements)){
									echo '<ul>';
									foreach ($hotel_dates_supplements as $key => $valuesup) {
										$valuesup->price_type = !empty($valuesup->price_type)?$valuesup->price_type:'pppn';
										$price = '';$price_euro_in = '';$price_out = '';$price_euro_out = '';

										if($prefer_currency == 1)
										{
											$price = !empty($valuesup->price)?'In : &pound;'.$valuesup->price:'';
											//$price_out = !empty($valuesup->price_out)?'Out : &pound;'.$valuesup->price_out:'';
										}
										else if($prefer_currency == 2)
										{
											$price_euro_in = !empty($valuesup->price_euro_in)?'In : &#8364;'.$valuesup->price_euro_in:'';
											//$price_euro_out = !empty($valuesup->price_euro_out)?'Out : &#8364;'.$valuesup->price_euro_out:'';
										}
										/*else
										{
											$price = !empty($valuesup->price)?'In -&pound;'.$valuesup->price:'';
										...
										if($valuesup->supplements == 1){
											echo '<li>Water view (Sea/Lake/River) : <strong>'.$price.' '.$price_euro_in.' '.$price_out.' '.$price_euro_out.' '.$valuesup->price_type.'</strong></li>';
										}elseif($valuesup->supplements == 2){
											echo '<li>View (Garden/Balcony) : <strong>'.$price.' '.$price_euro_in.' '.$price_out.' '.$price_euro_out.' '.$valuesup->price_type.'</strong></li>';
										}elseif($valuesup->supplements == 3){
											echo '<li>Executive/De Luxe/Superior Rooms : <strong>'.$price.' '.$price_euro_in.' '.$price_out.' '.$price_euro_out.' '.$valuesup->price_type.'</strong></li>';
										}elseif($valuesup->supplements == 4){
											echo '<li>Dbl/tw room for sole : <strong>'.$price.' '.$price_euro_in.' '.$price_out.' '.$price_euro_out.' '.$valuesup->price_type.'</strong></li>';
										}
									}
									echo '</ul>';
								}else{
                     ...
                     <div class="tc_box_wrapper">
                        <div class="tc_box">
                            ...
                            <div class="body">
                                ...
                                <div id="mealContent">
                                    {!! !empty($experienceDate->hc_mean_arrangements)?nl2br($experienceDate->hc_mean_arrangements):(!empty($value->hc_mean_arrangements)?$value->hc_mean_arrangements:'') !!}
                    ...
                    <?php  if(!empty($_GET['type']) && $_GET['type'] == 'cancelled') { ?> 
                     <div class="tc_box_wrapper">
                        <div class="tc_box">
                ...
                <div class="signatures" style="margin-top: 10px;">
                    <?php if(isset($value->sign_name_hc) && !empty($value->sign_name_hc)  && !isset($_GET['contracts'])){ ?>
                    	<div class="signature_column">
							...
							<p>
								<?php echo $value->sign_name_hc; ?><br>
								<?php 
             ...
             <div class="section mt-5">
            	...
            	<div class="row">
            		<a class="saveAllChangesBtn  yellowClrBtn marginTop15 clearBothCls float-left  " href="/super-user/stocklist-view-dates/{{$hotel_book_date_id}}/{{$hotel_id}}/{{!empty($hotel_cart_list[0]->id)?$hotel_cart_list[0]->id:'0'}}?type=download" style="font-size: 10px;">Download</a>
            	</div>
        </div>
    </div>
</div>
