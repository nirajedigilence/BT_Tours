    ...
    <div class="_contentBooking">
            ...
            <div class="filters">
                ...
                <div class="filters_dropdown">
                    ...
                    <div class="dropdown">
                        ...
                        <?php } else { ?> 
                            <form action="{{ route('acc-invoice') }}" id="filterForm">
                        <?php } ?>
                           ...
                           <input type="hidden" name="stage" id="stage" value="<?php echo isset($_GET['stage']) ? $_GET['stage'] : '' ?>" >
                            ...
                            <input type="hidden" name="need_action" id="need_action" value="<?php echo isset($_GET['need_action']) ? $_GET['need_action'] : 'no' ?>">
                            ...
                            <div class="dropdown_section">
                                ...
                                <div class="other_filters">
                                    ...
                                    <a href="javascript:;" class="option refnoClick <?php echo (isset($_GET['ref_no']) && $_GET['ref_no'] == 'no') ? '' :'active'; ?>"; ?>
                                        Ref no.
                                    </a>
                                    ...
                                    <a href="javascript:;" class="option <?php echo (isset($_GET['need_action']) && $_GET['need_action'] == 'yes') ? 'active' :''; ?> needActionClick">
                                        Show Tours Needing Action
                                    </a>
                ...
                <ul class="active_filters">
                    <li class="label">Active filters:<br>
                        <a href="{{ route('acc-invoice') }}" style="color: orange;font-weight: normal;text-decoration: underline !important;float: left;">reset</a>
                    ...
                    <?php } ?>
                    <?php if(isset($_GET['need_action']) && $_GET['need_action'] == 'yes'){ ?>
                        <li>Tours Need Action <a href="javascript:;" id="needActionRemove"><span>x</span></a></li>
                    <?php } ?>
                    <?php if(isset($_GET['hotel']) && $_GET['hotel'] == 'no'){ ?>
                    <?php }else{ ?>
                        <li>Hotel <a href="javascript:;" id="hotelRemove"><span>x</span></a></li>
                    <?php } ?>
                    <?php if(isset($_GET['ref_no']) && $_GET['ref_no'] == 'no'){}else{ ?>
                    ...
                    <?php } ?>
                    <?php if(isset($_GET['collaborator']) && $_GET['collaborator'] == 'no'){ }else{?>
        ...
        <style type="text/css">
          ...
          <div class="middleCol_row header">
                ...
                <div class="column width_small <?php echo (isset($_GET['ref_no']) && $_GET['ref_no'] == 'no') ? 'hide' :''; ?>" >
                    Ref No 
                </div>
                ...
                <div class="sorting column <?php echo (isset($_GET['collaborator']) && $_GET['collaborator'] == 'no') ? 'hide' :''; ?>"  data-sorting_type="desc" data-column_name="colobratorName" style="cursor: pointer">
                ...
                <div class="sorting column" data-sorting_type="desc" data-column_name="experience_name" style="cursor: pointer">
                    Tour Name <span class="sort-sp" id="experience_name_icon"><i class="fa fa-sort" aria-hidden="true"></i></span>
        ...
        <div id="ctour_list">
            @include ('partials.booking.current_tour_list_invoice')
        ...
        <div class="tableWrapper drag">
            ...
            <script>
                $(".openBooking").bind("click", function(){
                    ...
                    var history_page = '<?=!empty($history_page)?'1':'0'?>';
                    $(".openBooking").removeClass("active");
                    $(this).addClass("active");
                    $(".bookingForm").hide();
                    //$("#rightColInfo-"+cartExperienceId).show();
                    $.ajax({
                       url:"/get_right_col_invoice?id="+cartExperienceId+"&history_page="+history_page,
                       success:function(data)
                       {
                        $('#rightCol_div').html('');
...
<script>
    <?php
    $update_invoice_cart = Session::get('update_invoice_cart');
    if(!empty($update_invoice_cart))
    {
        ?>
        var cart_id = '<?=$update_invoice_cart?>';
            $('#openBooking-'+cart_id).trigger('click');
        <?php
    }
    ?>
...
$("body").on("click", '.showHideExep', function(e) {
    ...
    $("body").on("click", '#tournoRemove', function(e) {
     ...
     $("body").on("click", '#needActionRemove', function(e) {
        $('#need_action').val('no');
        $('#submitBtn').click();
    });
    ...
    $("body").on("click", '.needActionClick', function(e) {
        $(this).toggleClass('active');
        if($(this).hasClass("active")){
            $('#need_action').val('yes');
        }else{
...
$(document).ready(function(){
     function fetch_data(sort_type, sort_by, stage, ref_no,next_step,next_step_due,collaborator,experience,hotel,tour_no,month,date_from,date_to,need_action)
     {
        $('.loader').show();
        var history_page = '<?=!empty($history_page)?'1':'0'?>';
      $.ajax({
       url:"/fetch_current_tour_invoice?sortby="+sort_by+"&sorttype="+sort_type+"&stage="+stage+"&ref_no="+ref_no+"&next_step="+next_step+"&next_step_due="+next_step_due+"&collaborator="+collaborator+"&experience="+experience+"&hotel="+hotel+"&tour_no="+tour_no+"&month="+month+"&date_from="+date_from+"&date_to="+date_to+"&history_page="+history_page+"&need_action="+need_action,
       success:function(data)
       {
        $('#ctour_list').html('');
     ...
     $('body').on('click', '.sorting', function(){
        $('.bookingForm').hide();
          ...
          var need_action = $('#need_action').val();
          fetch_data(reverse_order, column_name, stage, ref_no,next_step,next_step_due,collaborator,experience,hotel,tour_no,month,date_from,date_to,need_action);
