...
$e_dates = array();
            if($hotel_type == 2)
            {
                $overnight_data = $cart_experience->overnight_outbound;
            }
            else{
                $overnight_data = $cart_experience->overnight_inbound;
            }
...
<div class="screen_5" style="display:block;">
     <div class="changeExpNew">
                      ...
                      <p>Select hotel to add</p>
                            ...
                            {{csrf_field()}}
                            <?php 
                            if(!empty($overnight_data)){
                                for($i=1;$i<=$overnight_data;$i++)
                                {?>
                                
                            <div class="form-group">
                                <label style="color:#000" class="label">Overnight {{$i}}</label>
                            <select class=" form-control expSelect_{{$i}} expChange hotelDropDown select2" name="hotel_name" id="hotel_name_{{$i}}"  required="" data-msg-required="Please select hotel" data-old-experience="{{$i}}">
                                     <option value="">Select One</option>
                                        <?php 
                                            if(!empty($hotelList)){
                                            foreach ($hotelList as $kk => $val) {
    ...
    <p>&nbsp;</p>
        ...
        <div class="tc_box_wrapper new_date" style="display:none;">
            <div class="tc_box">
                ...
                $overnight_outbound = $overnight_data;
                ...
                else
                {
                    ...
                    if(!empty($overnight_data)){
                        for($i=1;$i<=$overnight_data;$i++)
                        {
                            $cnt = ($i!=1)?$overnight_data-1:'0';
                             $crossing_outbound_end_date = date('Y-m-d', strtotime($crossing_outbound_end_date. ' - '.$cnt.' days'));
                            ?>
                <div class="body" id="new_date_div">
                   <p><span class="new_exp">Hotel name : <b><span class="new_hotel_name_{{$i}}"></span></b><span></p>
                   <p><span class="new_exp">Hotel Date : <b><span class="new_hotel_date">{{$crossing_outbound_end_date}}</span></b><span></p>
        ...
        var cnt =1;
           <?php 
                if(!empty($overnight_data)){
                    ...
                    {?>
                        var i = '<?=$i?>';
        var hotel_name = $('#hotel_name_'+i+' option:selected').text();
        var hotel_id = $('#hotel_name_'+i).val();
...
*/
        ...
        $('.screen_6 #add_new_hotel_'+i).val($('#hotel_name_'+i).val());
        var hotel_id = $('#hotel_name_'+i).val();
        //$('.screen_6 #add_new_hotel_type').val(hotel_type);
        //$('.screen_6 #add_new_exp_type').val(experience_type);
        var html = '<input type="hidden" name="add_new_hotel_'+i+'" id="add_new_hotel_'+i+'" value="'+hotel_id+'" class="btn btn-warning btn-sm ml-2">';   
        $('#hoteldiv').append(html);  
        $('.screen_6 #new_date_div .new_hotel_name_'+i).text(hotel_name);
        ...
        return false;
            ...
            else
            {
                ...
                //var params = $("#dataHolder").serialize();
                 let formData = new FormData(this);
                $.ajax({
                    url: "{{ route('add-hotel-amend-cruise') }}",
                    type: 'POST',
                    data: formData,
                    contentType: false,
                    processData: false,
