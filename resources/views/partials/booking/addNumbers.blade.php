			    	...
			    	<div class="form-check sup_{{$value->id}}">
			    		<div class="checkarea_part_Dates">
			                <label class="checkbox_div graycls" style="font-size: 13px;">
			                  <input type="checkbox" name="room_requests[{{$i}}][]" class="custom_chkbox tagchkbox notClickedCls {{$cls_dbl}}" data-id="{{$i}}" value="{{ $value->id }}"> {{ $sup }} <?php if ( Auth::check() && Auth::user()->hasRole("Super Admin")){ ?><b> In </b>({{$currency_symbol}}{{$price_in[$k]}}) {{$price_type[$k]}}-  <b>Out</b> <?php } ?>({{$currency_symbol}}{{$price_out[$k]}}) - {{($price_type[$k] == 'pppn')?'pp':$price_type[$k]}}<span class="notClickedCls "></span>
