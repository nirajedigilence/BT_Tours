...
DB::connection('mysql_veenus')->table('hotel_dates')->where('date_in',$cart_experience[0]->date_from)->where('date_out',$cart_experience[0]->date_to)->where('deleted_at',null)->first();
	...
	//multiple hotel
	$hotel_data = get_hotel_date($cart_experience[0]->id);
	            	...
	            	<ul class="nav tabs" role="tablist" style="margin: unset;">
	            		...
	            		if(!empty($experienceDates) && count($experienceDates) > 1){
							    foreach ($experienceDates as $key => $value) { 
	                                ...
	                                {
								    	$roomcounts = get_total_room($cart_experience[0]->id,$value['id']);
...
DB::connection('mysql_veenus')->table('hotel_dates')->select('h.id','h.name')->leftjoin('hotels as h','h.id','=','hotel_dates.hotel_id')->where('hotel_dates.id', $value['hotel_date_id'])->first();
	            		...
	            		?>
						<!-- @if(in_array('2',$exp_type_ar) || in_array('3',$exp_type_ar))
						...
						@if(in_array('1',$exp_type_ar) || in_array('3',$exp_type_ar))
							@if(in_array('2',$exp_type_ar) && !empty($hotel_array[2]))
					    	  <li style="width:25%;" class="nav-item " data-toggle="tab" href="javascript:;" id="secondary_list" role="tab">Secondary<br><p>(<?php echo implode(',', $hotel_array[2]);?>)</p>
									</li>
				...
				<div class="grouped_header">
	                <div class="header">
	                    ...
	                    <div class="main_details">
	                        ...
	                        <div class="detail">
	                            Date: {{ $hotel_data['date_from'] }} - {{ $hotel_data['date_to'] }} ( {{ $hotel_data['nights'] }} @if($hotel_data['nights'] > 1) nights @else night @endif )
	                        </div>
		    							...
		    							margin: 28px;">
								  		...
								  		<div class="row">
								  			<div class="col-sm-2" style=" padding: 0 50px; margin: 0 0 30px; ">
									  	 		<a data-fancybox data-type="ajax" data-src="" href="{{ route('add-multi-rooms', $cart_experience[0]->id) }}" class="btn btn-primary">
					                                Add a Room Request
					                            </a>
									  	 	</div>
								  		</div>
...
DB::connection('mysql_veenus')->table('users')->where('id', $evalue->user_id)->first();
	                    						...
	                    						@else
												  ...
												  <thead>
												    <tr>
												      ...
												      <th scope="col">@hasrole('Collaborator')Guest request @else Guest request @endhasrole </th>
												  ...
												  <tbody class="tblBorderCls">
												  	...
												  	foreach ($cartExperiencesToRooms as $key => $value) {
													  	...
													  	$paid_row = ($value->paid == 1 ||$value->deposit == 1  )?'paid_row': '';
												  	 ?>
												  	 @hasrole('Collaborator')
												  	 <?php
														...
														if($today >= $minusDays){
															if(!empty($value->is_optional_room)){
																	...
																	echo '<tr class='.$paid_row.'>';
																	}else{
																			echo '<tr class="table table-disaled '.$paid_row.'" >';
																	}
														}else{
															echo '<tr class='.$paid_row.'>';
														}
														?>
												  		@else
												  		<tr class="<?=$paid_row?> table table-disaled" style="pointer-events: none;">
											  			@endhasrole
													      		...
													      		if(!empty($roomsSupplementsRequest)){
													      			...
													      			foreach ($roomsSupplementsRequest as $rskey => $rsvalue) {
													      				if($rsvalue['cart_experiences_to_rooms_id'] == $value->id){
													      					...
													      					$remove_link = '<a data-fancybox data-type="ajax" data-src="" href="/remove-supplements/'.$value->id.'/'.$HotelDatesID.'/'.$cart_experience[0]->id.'"  style="color: #8a909d;font-size: 14px;">X</a>';
														  	 ...
														  	 <?php
																...
																<td>
													      		...
													      		if(count($experienceDates) > 1)
													      		{
														      		...
														      		<!-- <a class="sk_cancel_btn removeRoomsIconCls btn" data-id="{{$value->id}}"><i class="fas fa-trash-alt"></i></a> -->
														      		...
														      		if(!empty($room_type_supplements)){
														      			foreach ($room_type_supplements as $rskey => $rsvalue) {
														      				...
														      				if($rsvalue['cart_experiences_to_rooms_id'] == $value->id){
														      					...
														      					if($rsvalue['room_requests_id'] == 1){
													      						;
															      						$rsvalue['supplements'] = 'Water view (Sea/Lake/River) : '.$rsvalue['upgrade_request_sup_status'].$remove_link;
															      					}elseif($rsvalue['room_requests_id'] == 2){
															      						$rsvalue['supplements'] = 'View (Garden/Balcony) : '.$rsvalue['upgrade_request_sup_status'].$remove_link;
															      					}elseif($rsvalue['room_requests_id'] == 3){
															      						$rsvalue['supplements'] = 'Executive/De Luxe/Superior Rooms : '.$rsvalue['upgrade_request_sup_status'].$remove_link;
															      					}elseif($rsvalue['room_requests_id'] == 4){
															      						$rsvalue['supplements'] = 'Dbl/tw room for sole : '.$rsvalue['upgrade_request_sup_status'].$remove_link;
															      					}elseif($rsvalue['room_requests_id'] == 7){
															      						$rsvalue['supplements'] = 'Walk In Shower : '.$rsvalue['upgrade_request_sup_status'].$remove_link;
															      					}elseif($rsvalue['room_requests_id'] == 8){
															      						$rsvalue['supplements'] = 'Ground Floor Room : '.$rsvalue['upgrade_request_sup_status'].$remove_link;
															      					}elseif($rsvalue['room_requests_id'] == 5){
	                ...
	                <div class="download__print">
	                    ...
	                    <a data-fancybox="" data-type="ajax" data-src="" href="{{route('download-guest-list-multiple',$cart_experience[0]->id)}}" class="cta">
					                               Download
					                            </a>
                		@else
	                    ...
	                    <!-- <a target="_blank" href="{{route('send-guest-list-pdf',$cart_experience[0]->id)}}" class="cta">
	                     ...
	                     <a data-fancybox="" data-type="ajax" data-src="" href="{{route('send-guest-list-pdf-email',$cart_experience[0]->id)}}" class="cta">
					                               Send to Hotel</a>
	                   <?php } ?>
