<?php 
	...
	if(isset($cart_experience->cruise_dates_rates_id)){

		$experience_dates = 


DB::connection('mysql_veenus')->table('experience_cruise_dates')->where('dates_rates_id',$cart_experience->cruise_dates_rates_id)->first();
...
DB::connection('mysql_veenus')->table('ship_dates')->where('date_in',$cart_experience->date_from)->where('date_out',$cart_experience->date_to)->first();
		}
...
DB::connection('mysql_veenus')->table('ship_dates')->where('id',$experience_dates->hotel_date_id)->first();
...
DB::connection('mysql_veenus')->table('ship_dates')->where('date_in',$cart_experience->date_from)->where('date_out',$cart_experience->date_to)->where('deleted_at',null)->first();
		if(!empty($hotel_dates)){
			$HotelDatesID = $hotel_dates->id;
		}
	}
	...
	foreach ($cartExperiencesToRoomsAll as $key => $value) {
		...
		if(!empty($cabinDetail)){
           ...
           {
           		$cabinCount[$cabinval->id] = isset($cabinCount[$cabinval->id])?$cabinCount[$cabinval->id]:'0';
           		$cabinCountAll[$cabinval->id] = isset($cabinCountAll[$cabinval->id])?$cabinCountAll[$cabinval->id]:'0';
           		$cabinCountOrg[$cabinval->id] =$cabinval->no_cabin;
		        if($value->room_id == $cabinval->id && $value->cancellation_request_status == null && ($value->deposit == '1' || $value->paid == '1')){
					$cabinCount[$cabinval->id] = $cabinCount[$cabinval->id]+1;
				}
				if($value->room_id == $cabinval->id && $value->cancellation_request_status == null){
					$cabinCountAll[$cabinval->id] = $cabinCountAll[$cabinval->id]+1;
				}
	...
	//multiple hotel
	$hotel_data = get_hotel_date($cart_experience->id);
...
<style type="text/css">
	...
	td.unselectableclick {
	    background: #f5f6fa !important;
	    pointer-events: none !important;
	}
	tr.unclick td:first-child{
		background: #f5f6fa !important;
		pointer-events: none !important;
	}
	tr.unclick td:nth-child(2){
		background: #f5f6fa !important;
		pointer-events: none !important;
	}
	tr.unclick td:nth-child(3){
		background: #f5f6fa !important;
		pointer-events: none !important;
	}
	tr.unclick td:nth-child(4){
		background: #f5f6fa !important;
		pointer-events: none !important;
	}
...
<div class="notIndexContainer " style="margin-bottom: 200px; width: 100%;">
        ...
        <div class="guestlist_container multiguestlist_html">
        	@hasrole('Super Admin')
            <div class="grouped_header">
            	<div class="tabs_wrapper  sk_Rooms" style=" margin-top: 20px;">               	
	            	<ul class="nav tabs" role="tablist" style="margin: unset;">
	            		<?php 
	            		$exp_type_ar = array();
	            		$hotel_array = array();
	            		$over_night = 0;
	            		...
	            		if(!empty($experienceDates) && count($experienceDates) > 1){
							    foreach ($experienceDates as $key => $value) { 
	                                ...
	                                if($value['experience_type'] == $experience_type)
	                                {
	                                	$active_cls = ' active'; 
	                                }
							    	if($value['experience_type'] != '1')
	                                {
								    	$roomcounts = get_total_room($cart_experience->id,$value['id']);
								    	if($sngLastAmt != $roomcounts['single'])
								    	{
								    		$sngSCntstr1 .= ($value['experience_type'] == 2)?"Secondary ".$secondary.", ":"Overnight ".$over_night.", ";
								    	}
								    	if($dblLastAmt != $roomcounts['double'])
								    	{
								    		$dblSCntstr1 .= ($value['experience_type'] == 2)?"Secondary ".$secondary.", ":"Overnight ".$over_night.", ";
								    	}
								    	if($twnLastAmt != $roomcounts['twin'])
								    	{
								    		$twnSCntstr1 .= ($value['experience_type'] == 2)?"Secondary ".$secondary.", ":"Overnight ".$over_night.", ";
								    	}
								    	if($trpLastAmt != $roomcounts['triple'])
								    	{
								    		$trpSCntstr1 .= ($value['experience_type'] == 2)?"Secondary ".$secondary.", ":"Overnight ".$over_night.", ";
								    	}
								    	if($qrdLastAmt != $roomcounts['qurd'])
								    	{
								    		$qrdSCntstr1 .= ($value['experience_type'] == 2)?"Secondary ".$secondary.", ":"Overnight ".$over_night.", ";
								    	}

								    	if(!empty($roomcounts['singleLink']))
								    	{
								    		$sngSCntLinkstr1 .= ($value['experience_type'] == 2)?"Secondary ".$secondary.", ":"Overnight ".$over_night.", ";
								    	}
								    	if(!empty($roomcounts['doubleLink']))
								    	{
								    		$dblSCntLinkstr1 .= ($value['experience_type'] == 2)?"Secondary ".$secondary.", ":"Overnight ".$over_night.", ";
								    	}
								    	if(!empty($roomcounts['twinLink']))
								    	{
								    		$twnSCntLinkstr1 .= ($value['experience_type'] == 2)?"Secondary ".$secondary.", ":"Overnight ".$over_night.", ";
								    	}
								    	if(!empty($roomcounts['tripleLink']))
								    	{
								    		$trpSCntLinkstr1 .= ($value['experience_type'] == 2)?"Secondary ".$secondary.", ":"Overnight ".$over_night.", ";
								    	}
								    	if(!empty($roomcounts['qurdLink']))
								    	{
								    		$qrdSCntLinkstr1 .= ($value['experience_type'] == 2)?"Secondary ".$secondary.", ":"Overnight ".$over_night.", ";
								    	}
							    	}
							    	
							    	/*if(!empty($value['hotel_date_id']))
	                                {
	                                    $hotel_date = 


DB::connection('mysql_veenus')->table('hotel_dates')->select('h.id','h.name')->leftjoin('hotels as h','h.id','=','hotel_dates.hotel_id')->where('hotel_dates.id', $value['hotel_date_id'])->first();
	                                }
	                                if($value['experience_type'] == '3')
	                                {
	                                	$over_night++;
	                                }
							    	$exp_type_ar[]=$value['experience_type'];
							    	if(!empty($hotel_date))
							    	{
							    		$hotel_array[$value['experience_type']][]= $hotel_date->name;
							    	
							    	}*/
							    	?>
							    	 <li style="width:25%;" class="nav-item multiguestlist {{$active_cls}} expcls{{$value['id']}}" data-exp-id="{{$value['id']}}"  data-hotel-name="{{$value['hotel_name']}}" data-hotel-id="{{$value['hotel_id']}}"  data-exp-type="{{$value['experience_type']}}" data-toggle="tab" href="javascript:;"  role="tab"><?=($value['experience_type'] == 1)?'Primary Guest List':(($value['experience_type'] == 2)?'Secondary '.($secondary != '1'?$secondary:''):'Overnight '.$over_night )?><br><p>(<?php echo $value['hotel_name'];?>)</p>
									</li>
							    	<?php
	            		...
	            		?>
						<?php /* @if(in_array('2',$exp_type_ar) || in_array('3',$exp_type_ar))
							@if(in_array('1',$exp_type_ar) && !empty($hotel_array[1]))
					    	  <li style="width:25%;" class="nav-item " data-toggle="tab" href="javascript:;" id="primary_guest_list" role="tab">Primary Guest List<br><p>(<?php echo implode(',', $hotel_array[1]);?>)</p>
									</li>

							@endif
						@endif
						@if(in_array('1',$exp_type_ar) || in_array('3',$exp_type_ar))
							@if(in_array('2',$exp_type_ar) && !empty($hotel_array[{{$cabinval->id}}]))
					    	  <li style="width:25%;" class="nav-item " data-toggle="tab" href="javascript:;" id="secondary_list" role="tab">Secondary<br><p>(<?php echo implode(',', $hotel_array[{{$cabinval->id}}]);?>)</p>
									</li>

							@endif
						@endif
						@if(in_array('1',$exp_type_ar) || in_array('2',$exp_type_ar))
							@if(in_array('3',$exp_type_ar) && !empty($hotel_array[3]))
					    	  <li style="width:25%;" class="nav-item " data-toggle="tab" href="javascript:;" id="overnight_list1" role="tab">Overnight1<br><p>(<?php echo $hotel_array[3][0];?>)</p>
									</li>
							<?php if($over_night > 1) {?>
							 <li style="width:25%;" class="nav-item " data-toggle="tab" href="javascript:;" id="overnight_list2" role="tab">Overnight2<br><p>(<?php echo $hotel_array[3][1];?>)</p>
									</li>
								<?php } ?>
							@endif
						@endif */ ?>
				...
				<div class="grouped_header">
	                <div class="header">
	                    ...
	                    <div class="main_details">
	                        ...
	                        <div class="detail">
	                            Ship: {{ $ship_data['hotel_name'] }}
	                        </div>

	                        <div class="detail">
	                            Date: {{ $ship_data['date_from'] }} - {{ $ship_data['date_to'] }} ( {{ $ship_data['nights'] }} @if($ship_data['nights'] > 1) nights @else night @endif )
	                        </div>
...
DB::connection('mysql_veenus')->table('experiences')->where('id', $cart_experience->experiences_id)->first();
	                   $users_details = 
...
DB::connection('mysql_veenus')->table('users_details')->where('user_id',$cart_detail->user_id)->first();
	                    ...
	                    <div class="contact_info">
	                        ...
	                        <strong>direct: <span class="number"><?php 
	                                                if(!empty($detail)){
	                                                    echo '<a href="tel:'.str_replace(' ', '', $detail['contact_number']).'" style="color:#FCA311;">'.$detail['contact_number'].'</a>'; 
	                                                }
	                                                ?></span></strong><br>

	                        <strong>main: <span class="number"><?php echo isset($users_details->main_contact_number) ? '<a href="tel:'.str_replace(' ', '', $users_details->main_contact_number).'" style="color:#FCA311;">'.$users_details->main_contact_number.'</a>' : ''; ?></span></strong>
	                    </div>
	                    @endhasrole

	                </div>
	                <?php
	                 $cartdata = App\Models\Cms\Cart::where("id", $cart_experience->carts_id)->first();
	                 $today = strtotime("today");
					$minusDays = strtotime('-28 days',strtotime($cart_experience->date_from));
	                ...
	                <div class="sub_header">
	                    ...
	                    <ul class="nav">
	                     ...
	                     <!--   <li style="width:auto;">
	                            <a data-fancybox data-type="ajax" data-src="" href="{{ route('swap-rooms', $cart_experience->id) }}" class="cta">
	                                Room Swap
	                            </a>
	                        </li> -->
							...
							if($today >= $minusDays){
								if(!empty($value->is_optional_room)){
												...
												?>												
												<li style="width:auto;">
						                            <a data-fancybox data-type="ajax" data-src="" href="{{ route('cancel-rooms', $cart_experience->id) }}" class="cta">
	                        ...
	                        if(strtotime($cart_experience->date_from) > $today)
	                        {
	                        	 if($today >= $minusDays){
							...
							?>
	                         <li style="width:auto;">
	                            <a data-fancybox data-type="ajax" data-src="" href="{{ route('add-rooms', $cart_experience->id) }}" class="cta">
	                                Request Additional Room
	                            </a>
	                        </li>
	                        <li style="width:auto;">
	                            <a data-fancybox data-type="ajax" data-src="" href="{{ route('general-enquiry', $cart_experience->id) }}" class="cta">
	                                General Enquiry
	                            </a>
	                        </li>
	                        <?php if(!empty($cart_experience->driver)){
	                        	?>
	                        	 <!-- <li style="width:auto;">
		                            <a data-fancybox="" data-type="ajax" data-src="" href="/add-driver/{{$cart_experience->id}}" class="cta">Add Driver Detail</a>
		                        </li> -->
		                        	
		                        	<?php 
		                        } ?>
	                    	<!-- <li class="large">
	                            <a class="cta tourOverviewButton" data-fancybox data-type="ajax" data-src="" href="{{ route('add-rooms', $cart_experience->id) }}">
	                        ...
	                        <li>
	                        	<?php
	                        		$cartExperiencesToRoomsRequestCount = App\Models\Cms\CartExperiencesToCabins::with('cartExperiencesToRoomsRequests')->whereIn('room_request_status',['pending'])->where("cart_exp_id", $cart_experience->id)->orderBy('date', 'DESC')->count();
	                        	?>
	                        	<?php  if(!empty($experienceDates) && count($experienceDates) > 1){ ?> 
	                        		 <a href="javascript:;" class="cta sktrigger" data-tab="sk_MultiRequests">
...
DB::connection('mysql_veenus')->table('cabins_supplements_requests')->where('cart_id', $cart_experience->id)->where('upgrade_request_sup_status','pending')->groupBy('cart_experiences_to_rooms_id')->count();
	                        	?>
	                       ...
	                       <!--   <li>
	                        ...
	                        <li class="large">
	                            <a href="javascript:;" class="cta sktrigger" data-tab="sk_cancellations">
	                                Cancellations(<?=$cartExperiencesToRoomsCancellationCount?>)
	                            </a>
	                        </li>
...
DB::connection('mysql_veenus')->table('general_enquiries')->where('cart_experience_id', $cart_experience->id)->where('is_resolved','0')->count();
	                        	?>
	                    ...
	                    <div class="tour_sales">
	                    	...
	                    	foreach ($cartExperiencesToRoomsAll as $key => $value) {
	                    		//pr($value);
	                    		if(($value->paid == 1 || $value->deposit == 1) && $value->cancellation_request_status != 'approved'){
	                    			$ple = 1;
	                    			/*if($value->room_id == 1)
	                    			{
	                    			 	$ple = 1;
	                        ...
	                        <div>Cabin sold: {{$rooms_sold+$d_cnt}}</div>
	                	...
	                	<ul class="nav tabs" role="tablist" style="margin-top: 15px;">
						  	<!-- @if ($sngLastAmt > 0)
						   	...
						   	@endif -->
						   	  @if(!empty($cabinDetail))
                               @foreach($cabinDetail as $cabinval)
                               @if(!empty($cabinCountAll[$cabinval->id]))
                               <li class="nav-item {{(!empty($LastAddedRequest) && $LastAddedRequest->room_id == 1)?'active':(empty($LastAddedRequest)?'active':'')}}" data-toggle="tab" href="javascript:;" data-href="#cabin{{$cabinval->id}}" role="tab">{{$cabinCount[$cabinval->id]}}/{{$cabinCountAll[$cabinval->id]}})
								</li>
								@endif
	                            @endforeach
	                        @endif
							...
							<li class="nav-item" data-toggle="tab" href="javascript:;" data-href="#driverTab" role="tab">Driver (<?php echo ($cart_experience->driver_name != '' && $cart_experience->driver_name1 != '') ? '2' : (($cart_experience->driver_name != '') ? '1' : '0'); ?>/<?=!empty($cart_experience->driver_name1)?'2':'1'?>)
							</li>
						...
						@else
							<ul class="nav tabs" role="tablist">
                               ...
                               @foreach($cabinDetail as $k1=>$cabinval)
                                @if(!empty($cabinCountAll[$cabinval->id]))
                               <li class="nav-item  {{($k1 == 0)?'active':''}} " data-toggle="tab" href="javascript:;" data-href="#cabin{{$cabinval->id}}" role="tab">{{$cabinval->name}} ({{$cabinCount[$cabinval->id]}}/{{$cabinCountAll[$cabinval->id]}})
								</li>
								@endif
	                            @endforeach
	                        @endif
							@if ($sngLastAmt > 0 || $dblLastAmt > 0 || $twnLastAmt > 0 || $trpLastAmt > 0 || $qrdLastAmt > 0 )
						   	...
						   	@endif
						  	@if ($sngLastAmt > 0)
								<li class="nav-item {{(!empty($LastAddedRequest) && $LastAddedRequest->room_id == 1)?'active':(empty($LastAddedRequest)?'active':'')}}" data-toggle="tab" href="javascript:;" data-href="#SignleRooms" role="tab">
								<?php if(!empty($sngSCntstr1)){ ?>

									<span data-tooltip="<?php
										echo "This room is not linked. " ."\n"."Diffrent allocation on :"."\n".rtrim($sngSCntstr1, ', '); ?>" data-tooltip-position="top"><i style="color:red;" class="fa fa-exclamation-triangle" aria-hidden="true"></i></span>

								<?php } ?>
								
								Single Rooms ({{$sngSCnt}}/{{$sngLastAmt}})
								</li>
						   	@endif
							
						  	@if ($dblLastAmt > 0)
						  		
								<li class="nav-item  {{(!empty($LastAddedRequest) && $LastAddedRequest->room_id == 2)?'active':''}}" data-toggle="tab" href="javascript:;" data-href="#DoubleRooms" role="tab">

									<?php if(!empty($dblSCntstr1)){ ?>
									<span data-tooltip="<?php
										echo "This room is not linked. " ."\n"."Diffrent allocation on :"."\n".rtrim($dblSCntstr1, ', '); ?>" data-tooltip-position="top"><i style="color:red;" class="fa fa-exclamation-triangle" aria-hidden="true"></i></span>

									<?php } ?>
									
									Double Rooms ({{$dblSCnt}}/{{$dblLastAmt}})
								</li>
						   	@endif
						  	@if ($twnLastAmt > 0)
								<li class="nav-item {{(!empty($LastAddedRequest) && $LastAddedRequest->room_id == 3)?'active':''}}" data-toggle="tab" href="javascript:;" data-href="#TwinRooms" role="tab">
									<?php if(!empty($twnSCntstr1)){ ?>
									<span data-tooltip="<?php
										echo "This room is not linked. " ."\n"."Diffrent allocation on :"."\n".rtrim($twnSCntstr1, ', '); ?>" data-tooltip-position="top"><i style="color:red;" class="fa fa-exclamation-triangle" aria-hidden="true"></i></span>

									<?php } ?>
									
									Twin Rooms ({{$twnSCnt}}/{{$twnLastAmt}})
								</li>
						   	@endif
							@if ($trpLastAmt > 0)
								<li class="nav-item {{(!empty($LastAddedRequest) && $LastAddedRequest->room_id == 4)?'active':''}}" data-toggle="tab" href="javascript:;" data-href="#TripleRooms" role="tab">
									<?php if(!empty($trpSCntstr1)){ ?>
									<span data-tooltip="<?php
										echo "This room is not linked. " ."\n"."Diffrent allocation on :"."\n".rtrim($trpSCntstr1, ', '); ?>" data-tooltip-position="top"><i style="color:red;" class="fa fa-exclamation-triangle" aria-hidden="true"></i></span>

									<?php } ?>
									
									Triple Rooms ({{$trpSCnt}}/{{$trpLastAmt}})
								</li>
							@endif
							@if ($qrdLastAmt > 0)
								<li class="nav-item {{(!empty($LastAddedRequest) && $LastAddedRequest->room_id == 5)?'active':''}}" data-toggle="tab" href="javascript:;" data-href="#QuadrupleRooms" role="tab">
									<?php if(!empty($qrdSCntstr1)){ ?>
									<span data-tooltip="<?php
										echo "This room is not linked. " ."\n"."Diffrent allocation on :"."\n".rtrim($qrdSCntstr1, ', '); ?>" data-tooltip-position="top"><i style="color:red;" class="fa fa-exclamation-triangle" aria-hidden="true"></i></span>

									<?php } ?>

									
									Quadruple Rooms ({{$qrdSCnt}}/{{$qrdLastAmt}})
								</li>
							@endif
	            ...
	            <div class="main">
	            		...
	            		<!-- Tab panes -->
	            		<?php  if(!empty($experienceDates) && count($experienceDates) > 1){ ?> 
	            			<div class="tab-content skalltabs sk_MultiRequests" style="display: none;">
								@if ($sngLastAmt > 0)
								  	<div class="-tab-pane active" id="RoomRequest" role="tabpanel" style="border: 1px solid;
		    							margin: 28px;">
								  		<h3  style="padding: 5px 38px;margin: 0 0 15px;color: #212529;">Cabin Requests</h3>
								  		<div class="row">
								  			<div class="col-sm-2" style=" padding: 0 50px; margin: 0 0 30px; ">
									  	 		<a data-fancybox data-type="ajax" data-src="" href="{{ route('add-multi-rooms', $cart_experience->id) }}" class="btn btn-primary">
					                                Add a Cabin Request
					                            </a>
									  	 	</div>
								  		</div>
...
DB::connection('mysql_veenus')->table('hotel_dates_supplements')->where('hotel_dates_id', $hotel_dates->id)->where('supplements', $valueCRR['room_requests_id'])->first();
									  			 ...
									  			 if(!empty($hotel_dates_supplements))
									  			 {
									  			 	 $hotel_dates_supplements->price_type = !empty($hotel_dates_supplements->price_type)?$hotel_dates_supplements->price_type:'pp';
		                                                    ...
		                                                    $price_out_sup = ($cart_experience->currency == 2)?$hotel_dates_supplements->price_euro_out:$hotel_dates_supplements->price_out;
									  	 ...
									  	 ?>	
									  	 	<div class="col-sm-12 mainSuperPanel view_req_div_<?php echo $value->id; ?>"  style=" padding: 0 50px; margin: 0 0 30px; ">								  		
												<!-- <form method="post" action="http://tours-system-com.stackstaging.com/add-rooms-detail" id="dataHolder" novalidate="novalidate"> -->
												    <div class="roomHolder superRequest" id="roomHolder1" style="display: block;float: left;width: 100%;">
												    	<h3>Cabin Request #<?php echo $s++; ?> <span style="float: right;"><?php echo date('H:i d/m/Y',strtotime($value->date)); ?></span></h3>
												  		<div class="form-group">
														    <label class="graycls">Cabin</label>
													    	<label class="supplement_title">
													    		
													    		<?php echo $roomname; ?> </label> 
												    	...
												    	<div class="form-group marginBottom0">
													  	...
													  	<div class="sk_actions<?php echo $rowId; ?> initialdiv">    
														  	...
														  	<div class="form-group " style="margin-top: 12px;clear: both;">
														  			<a href="javascript:;" data-id="<?php echo $value->id; ?>" class="cta save_room_req" style="width: 10%;float: left;">Save</a>
														  			<?php if(!empty($experienceDates) && count($experienceDates) > 1){ ?>
														  			<!-- <a href="javascript:;" data-id="<?php echo $value->id; ?>" class="cta edit_room_req" style="width: 10%;float: left;">Edit</a> -->
														  			<?php } ?>
									  	 	...
									  	 	<?php if(!empty($experienceDates) && count($experienceDates) > 1){ ?>
											<div style="display:none;" class="col-sm-12 mainSuperPanel edit_req_div_<?php echo $value->id; ?>" style=" padding: 0 50px; margin: 0 0 30px; ">								  		
												<!-- <form method="post" action="http://tours-system-com.stackstaging.com/add-rooms-detail" id="dataHolder" novalidate="novalidate"> -->
												    <div class="roomHolder superRequest" id="roomHolder1" style="display: block;float: left;width: 100%;">
												    	<h3>Cabin Request #<?php echo $s++; ?> <span style="float: right;"><?php echo date('H:i d/m/Y',strtotime($value->date)); ?></span></h3>
												  		<div class="form-group" style="clear:both;">
														    <label class="graycls">Cabin</label>
													    	<label class="supplement_title">
													    		<select name="room_type" data-id="" class="form-control room_type_<?php echo $rowId; ?>" data-msg-required="Please select type" required>
																	<option value="">Select</option>
																	<?php foreach (allRoomType() as $key => $valuerom) { ?>
																			<option <?=(!empty($roomname) && $roomname == $valuerom->name)?'selected':''?> value="{{ $valuerom->id }}">{{ $valuerom->name }}</option>
																	<?php } ?>
													     ...
													     <?php if(!empty($supplements)){ ?> 
													    	<div class="form-group marginBottom0" style="   clear: both;">
															    <label class="graycls">Request Type</label>
													    	...
													    	//pr($roomRequests);
													    	$cls_dbl = '';
													    	foreach ($roomRequests as $key => $value) { 
													    		if(!empty($supplements) && in_array($value->id, array_column($supplements, 'supplements'))){
													    			...
													    			if($cart_experience->currency == 2)
													    			{
													    				$price_out = array_column($supplements, 'price_euro_out');
													    				$price_in = array_column($supplements, 'price_euro_in');
													    			}
													    			else
													    			{
													    				$price_out = array_column($supplements, 'price_out');
													    				$price_in = array_column($supplements, 'price');
													    			}
													    			
													    			$price_type = array_column($supplements, 'price_type');
													    			$sup = '';
													    				if($value->id == 1){
													    		...
													    		?>
														    	<div class="form-check sup_{{$value->id}}">
														    		<div class="checkarea_part_Dates">
														                <label class="checkbox_div graycls" style="font-size: 13px;">
														                  <input type="checkbox" name="room_requests[]" class="custom_chkbox tagchkbox notClickedCls {{$cls_dbl}}" data-id="{{$rowId}}" value="{{ $value->id }}"> {{ $sup }} <?php if ( Auth::check() && Auth::user()->hasRole("Super Admin")){ ?><b> In </b>({{$currency_symbol}}{{$price_in[$k]}}) {{$price_type[$k]}}-  <b>Out</b> <?php } ?>({{$currency_symbol}}{{$price_out[$k]}}) - {{($price_type[$k] == 'pppn')?'pp':$price_type[$k]}}<span class="notClickedCls "></span>
															    ...
															    <div class="form-check" style="margin-left: 16px;">
														    		<div class="checkarea_part_Dates">
														                <label class="checkbox_div graycls">
														                  <input type="checkbox" name="room_requests_other[{{$rowId}}][]" class="custom_chkbox tagchkbox notClickedCls other_supplement" value="5"> Other<span class="notClickedCls "></span>
														                  <span class="checkmark"></span>
														                </label>
														            </div	>
														        </div>
													        </div>
													        <div class="add_div_{{$rowId}} other_div" style="display:none;">
													        	<button class="btn-sm btn-primary add_upgrade mt-5" type="button" data-value="{{$rowId}}" id="add_upgrade">Add More</button>
																<div class="guest_list_upgrade_other">
																	<!-- <input type="checkbox" id="other" /> <label for="other">Other:</label>  -->
																		...
																		<div class="guest_list_upgrade_other_control upgrade_name_div">
																			...
																			<input type="text" name="upgrade_name[{{$rowId}}][]" maxlength="100" id="upgrade_name" class="upgrade-name" placeholder="Additional upgrade" />
																		</div>
																		
																		<div class="guest_list_upgrade_other_control upgrade_cost_div">
																			<label>Upgrade cost IN {{$currency_symbol}}</label>
																			<input type="text" name="upgrade_cost[{{$rowId}}][]" maxlength="5" id="upgrade_cost" class="upgrade-cost" / placeholder=" " />
																		</div>
																		<div class="guest_list_upgrade_other_control upgrade_cost_out_div">
																			<label>Upgrade cost OUT {{$currency_symbol}}</label>
																			<input type="text" name="upgrade_cost_out[{{$rowId}}][]" maxlength="5" id="upgrade_cost_out" class="upgrade-cost-out" / placeholder=" " />
																		</div>
																		<div class="guest_list_upgrade_other_control upgrade_cost_type_div">
																			<select class="form-control notClickedCls valid" name="upgrade_cost_type[{{$rowId}}][]" class="upgrade_cost_type" fdprocessedid="24vei" aria-invalid="false">
													    	...
													    	Cabin requests: -need to know how we'll add up sums on this and see the designs
													    	<br /><br /> --}}
													    	<div class="guest_list_name guest_name_div_{{$rowId}}" style="display:block;">
													    		<div class="form-group marginBottom0">
																    <label class="graycls">Guest Names</label>
														  		...
														  		<div class="appendguestname">
														  			<div class="form-group guestinput_{{$rowId}} guest1_{{$rowId}}" style="display: inline-block;width: 100%;">
																	    <p>Guest Name 1</p>
															    		<input name="guest_name[{{$rowId}}][]" maxlength="100" class="form-control"></input>
														  			</div>
														  			<div class="form-group guestinput_{{$rowId}} guest2_{{$rowId}}" style="display: inline-block;width: 100%;">
																	    <p>Guest Name 2</p>
															    		<input name="guest_name[{{$rowId}}][]" maxlength="100" class="form-control"></input>
														  			</div>
														  			<!-- <div class="form-group guestinput_{{$rowId}} guest3_{{$rowId}}" style="display: inline-block;width: 100%;">
																	    <p>Guest Name 3</p>
															    		<input name="guest_name[{{$rowId}}][]" maxlength="100" class="form-control"></input>
														  			</div>
														  			<div class="form-group guestinput_{{$rowId}} guest4_{{$rowId}}" style="display: inline-block;width: 100%;">
																	    <p>Guest Name 4</p>
															    		<input name="guest_name[{{$rowId}}][]" maxlength="100" class="form-control"></input>
														  			</div> -->
														  		</div>
	            			...
	            			<div class="tab-content skalltabs sk_Requests" style="display: none;">
								  	...
								  	<div class="-tab-pane active" id="RoomRequest" role="tabpanel" style="border: 1px solid;
		    							margin: 28px;">
								  		...
								  		<div class="row">
								  			<div class="col-sm-2" style=" padding: 0 50px; margin: 0 0 30px; ">
									  	 		<a data-fancybox data-type="ajax" data-src="" href="{{ route('add-rooms-cruise', $cart_experience->id) }}" class="btn btn-primary">
					                                Add a Cabin Request
					                            </a>
									  	 	</div>
								  		</div>
								...
								if($cartExperiencesToRoomsRequest->count()>0){
									$s =1;
									  	foreach ($cartExperiencesToRoomsRequest as $key => $value) {
									  		...
									  		$roomname =  $value->cabinData->name;
									  		
									  		$ceTrR = [];
									  		$room_supplements = [];
										  	foreach ($value->cartExperiencesToCabinsRequests as $keyCRR => $valueCRR) {

									  			$ceTrR[] = $valueCRR->room_requests_id;
									  			 $hotel_dates_supplements = 


DB::connection('mysql_veenus')->table('ship_dates_supplements')->where('ship_dates_id', $hotel_dates->id)->first();
									  			 $pstr = '';
									  			 if(!empty($hotel_dates_supplements))
									  			 {
									  			 	 $hotel_dates_supplements->price_type = !empty($hotel_dates_supplements->price_type)?$hotel_dates_supplements->price_type:'pp';
		                                                    $hotel_dates_supplements->price_type  = ($hotel_dates_supplements->price_type == 'pppn')?'pp':$hotel_dates_supplements->price_type;
		                                                    $price_out_sup = ($cart_experience->currency == 2)?$hotel_dates_supplements->out_price_euro:$hotel_dates_supplements->out_price_euro;
		                                                    $pstr = !empty($price_out_sup)?' ('.$currency_symbol.$price_out_sup/*.' '.$hotel_dates_supplements->price_type*/.')':'';
									  			 }
									  					if($valueCRR['room_requests_id'] == 1){
								      						$valueCRR['supplements'] ='Single Document';
								      					}elseif($valueCRR['room_requests_id'] == 5){
								      						$upgrade_name =  explode(',',$valueCRR['upgrade_name']);
									      						$upgrade_cost =  explode(',',$valueCRR['upgrade_cost']);
									      						$upgrade_cost_out =  explode(',',$valueCRR['upgrade_cost_out']);
								      					...
								      					$valueCRR['supplements'] =$hotel_dates_supplements->supplement_name;
...
DB::connection('mysql_veenus')->table('hotel_dates_supplements')->where('hotel_dates_id', $hotel_dates->id)->where('supplements', $rrvalue['supplements'])->first();
													$pstr = '';
										  			 if(!empty($hotel_dates_supplements))
										  			 {
										  			 	 $hotel_dates_supplements->price_type = !empty($hotel_dates_supplements->price_type)?$hotel_dates_supplements->price_type:'pp';
			                                                    $hotel_dates_supplements->price_type  = ($hotel_dates_supplements->price_type == 'pppn')?'pp':$hotel_dates_supplements->price_type;
			                                                     $price_out_sup1 = ($cart_experience->currency == 2)?$hotel_dates_supplements->price_euro_out:$hotel_dates_supplements->price_out;
...
DB::connection('mysql_veenus')->table('general_enquiries')->where('cart_experience_id', $cart_experience->id)->get()->toArray();
								if(!empty($enquiries)){
...
DB::connection('mysql_veenus')->table('users')->where('id', $evalue->user_id)->first();
	            		...
	            		<form method="post" action="{{ route('colaborator-add-main-cabins') }}" id="dataUpdateNumberHolder" class=" skalltabs sk_Rooms">
							<input type="hidden" value="{{$cart_experience->id}}" id="current_cart_id" name="cart_id" class="cartIdSCls">
							@hasrole('Collaborator')
							...
							<div class="tab-content">
                               ...
                               @foreach($cabinDetail as $k1=> $cabinval)
								  	...
								  	<div class="tab-pane {{($k1 == 0)?'active':''}}" id="cabin{{$cabinval->id}}" role="tabpanel">
								  		<div class="row">
											<div class="col-sm-12">
										  		@hasrole('Collaborator')
										  			<?php
												...
												@else
												  ...
												  <thead>
												   <tr style="pointer-events: auto;">
												      <th scope="col">Guest status confirm </th>
												      <th scope="col">Name</th>
												      <th scope="col">Cabin request </th>
												  ...
												  <tbody class="tblBorderCls">
												  	...
												  	foreach ($cartExperiencesToRooms as $key => $value) {
												  		if($value->room_id == $cabinval->id && empty($value->is_optional_room)){
																...
																<?php if(!empty($value->is_optional_room)){
														      	...
														      	<div class="checkarea_part_Dates">
										                            <!-- <label class="checkbox_div">
										                              <input type="checkbox" class="custom_chkbox tagchkbox notClickedCls sk_con" value="1" name="roomPaidEdit[{{$cabinval->id}}][{{$value->id}}]" <?php echo $value->status == '1'?'checked': ''?>> <span class="notClickedCls"></span>
										                              <span class="checkmark"></span>
										                              <span class="sk_label">Provisional</span>
										                            </label> -->

										                            <label class="checkbox_div">
										                              <input type="checkbox" class="custom_chkbox tagchkbox notClickedCls sk_dep" value="1" name="roomDepositEdit[{{$cabinval->id}}][{{$value->id}}]" <?php echo $value->deposit == '1'?'checked': ''?>> <span class="notClickedCls"></span>
										                              <span class="checkmark"></span>
										                              <span class="sk_label">Deposit Paid</span>
										                            </label>

										                            <label class="checkbox_div">
										                              <input type="checkbox" class="custom_chkbox tagchkbox notClickedCls sk_paid" value="1" name="roomPaidFullEdit[{{$cabinval->id}}][{{$value->id}}]" <?php echo $value->paid == '1'?'checked': ''?>> <span class="notClickedCls"></span>
										                              <span class="checkmark"></span>
...
DB::connection('mysql_veenus')->table('ship_dates_supplements')->where('id', $rsvalue['supplements'])->first();
													      				...
													      				if($rsvalue['cart_experiences_to_rooms_id'] == $value->id){
													      					...
													      					$remove_link = '<a style="color:#8a909d;font-weight:700" href="#" class="remove_upgrade_sup" data-id="'.$rsvalue['id'].'"> X </a>';
													      					if($rsvalue['upgrade_request_sup_status'] == 'declined'){
															      						continue;
															      					}
													      					if($rsvalue['supplements'] == 5){
													      						$rsvalue['supplements'] = '<span>'.$rsvalue['upgrade_name'].' : '.$rsvalue['upgrade_request_sup_status'].'</span> '.$remove_link;
													      					}elseif($rsvalue['supplements'] == 1){
													      						$rsvalue['supplements'] = '<span> Single Document :'.$rsvalue['upgrade_request_sup_status'].'</span> '.$remove_link;
													      						$dbl_sole = 1;
													      					}else
													      					{
													      						$rsvalue['supplements'] = '<span>'.$supplementsdata->supplement_name.' : '.$rsvalue['upgrade_request_sup_status'].'</span> '.$remove_link;
													      					}
																...
																<input type="text" class="form-control" placeholder="Enter name" name="singleRoomNameEdit[{{$cabinval->id}}][{{$value->id}}][]" value="{{@$names[0]}}">
																<?php if(empty($dbl_sole)){ ?>
																<input type="text" <?=!empty($dbl_sole)?'disabled':''?> class="form-control" placeholder="Enter name" name="singleRoomNameEdit[{{$cabinval->id}}][{{$value->id}}][]" value="{{@$names[1]}}">
																 <?php } ?>
													      	</td>
													      	<?php
															$today = strtotime("today");
															$minusDaysUp = strtotime('-14 days',strtotime($cart_experience->date_from));
															...
															?>
													      		...
													      		if(!empty($supplements)){
													      			echo implode('<br>', $supplements);
													      			 if(!empty($value->upgrade_request_status)){
															      			if(empty($pen_can)){ ?>
															      			<br><a data-fancybox data-type="ajax" data-src="" href="/request-supplements-cruise/{{$value->id}}/{{$HotelDatesID}}/{{$cart_experience->id}}"  style="color: #FCA311;font-size: 14px;">Add</a>
															      				<?php } } 
													      		}else{
													      		?>
														      	...
														      	if(!empty($pen_can)){ ?>
														      		...
														      		<br><a class="cancelUpgrade" href="javascript:;" data-src="{{ route('request-supplements-cancel', $value->id) }}/?hotel_dates_id=<?php echo $HotelDatesID; ?>&cart_id=<?php echo $cart_experience->id; ?>" style="color: #FCA311;font-size: 14px;">Cancel Upgrade</a>
													      		<?php }}else{ ?>
													      			<br><a data-fancybox data-type="ajax" data-src="" href="/request-supplements-cruise/{{$value->id}}/{{$HotelDatesID}}/{{$cart_experience->id}}"  style="color: #FCA311;font-size: 14px;" class="up-request">Request</a>
													      		<?php } ?>
																 @else
																	<?php if(!empty($value->upgrade_request_status)){
																	if(!empty($pen_can)){ ?>
													      	...
													      	<td>
																<textarea class="form-control"  placeholder="Enter Note for the hotel" name="specialRequestsEdit[{{$cabinval->id}}][{{$value->id}}]">{{$value->specials}}</textarea>
													      	</td>
													      	<td>
													      		@hasrole('Collaborator')
													      		...
													      		@else
													      		<?php 
													      		if(count($experienceDates) > 1)
													      		{
													      			$check =  check_primary_room($value->id);
														      		$checklink =  check_linkprimary_room($value->id);
														      		
														      		if(empty($check) || $check < (count($experienceDates)-1)){?>
														      		<span data-tooltip="<?php
																				echo "This room is not linked. " ."\n"."Diffrent allocation on :"."\n".rtrim($dblSCntstr1, ', '); ?>" data-tooltip-position="top"><i style="color:red;" class="fa fa-exclamation-triangle" aria-hidden="true"></i></span>
																			<?php } else{
																			if(!empty($checklink)){?>
														      		<span data-tooltip="<?php
																				echo "This room is not linked. " ."\n"."Diffrent allocation on :"."\n".rtrim($checklink, ', '); ?>" data-tooltip-position="top"><i style="color:orange;" class="fa fa-unlink" aria-hidden="true"></i></span>
																		<?php } }
													      		}?>
														      	...
														      	<td scope="row">
															      	<div class="checkarea_part_Dates ">
																		...
																		<div>
																			...
																			<!-- <label class="checkbox_div">
												                              <input type="checkbox" class="custom_chkbox tagchkbox notClickedCls " value="1" name="roomPaidEdit[{{$cabinval->id}}][{{$value->id}}]" <?php echo $value->status == '1'?'checked': ''?>> <span class="notClickedCls"></span>
												                              <span class="checkmark"></span>
												                            </label> -->
												                            <div class="checkarea_part_Dates">
																		...
																		<?php
													      					...
													      					<div class="sk_status_change_{{$value->id}}">
													      						...
													      						<br>
																				<label>Guest request pending...</label>
...
DB::connection('mysql_veenus')->table('ship_dates_supplements')->where('id',$valueCRR['room_requests_id'])->first();
													      					...
													      					elseif($valueCRR['room_requests_id'] == 1){
													      					...
													      					{
													      						$valueCRR['supplements'] = !empty($supplementsdata->supplement_name)?$supplementsdata->supplement_name:'';
													      					}
														      	...
														      	<?php
																	...
																	<?php /*if(!empty($value->upgrade_request_status)){ */?>
														      		...
														      		if(!empty($room_type_supplements)){
														      			foreach ($room_type_supplements as $rskey => $rsvalue) {
														      				...
														      				if($rsvalue['cart_experiences_to_rooms_id'] == $value->id){
													      						...
													      						;
															      						$rsvalue['supplements'] = 'Single Document : '.$rsvalue['upgrade_request_sup_status'].$remove_link;
														      					}elseif($rsvalue['room_requests_id'] == 5){
												      						$rsvalue['supplements'] = $rsvalue['upgrade_name'].' : '.$rsvalue['upgrade_request_sup_status'].$remove_link;
													      					}
															      	...
															      	if(!empty($pen_can)){ ?>
															      		<br><i>Upgrade Request <?php echo ucwords($value->upgrade_request_status); ?></i>
															      		<br><a class="cancelUpgrade" href="javascript:;" data-src="{{ route('request-supplements-cancel', $value->id) }}/?hotel_dates_id=<?php echo $HotelDatesID; ?>&cart_id=<?php echo $cart_experience->id; ?>" style="color: #FCA311;font-size: 14px;">Cancel Upgrade</a><br>
														      		<?php } }else{ ?>
														      			<br><a data-fancybox data-type="ajax" data-src="" href="/request-supplements-cruise/{{$value->id}}/{{$HotelDatesID}}/{{$cart_experience->id}}"  style="color: #FCA311;font-size: 14px;" class="up-request">Request</a><br>
														      		<?php } ?>
																	 @else
																		<?php if(!empty($value->upgrade_request_status)){
																		if(!empty($pen_can)){  ?>
														      	...
														      	<td>
														      		<a data-fancybox data-type="ajax" data-src="" href="/remove-room/{{$value->id}}"  style="color: #8a909d;font-size: 14px;"><i class="fas fa-trash-alt"></i></a>
														      	</td>
														    </tr>
													  	<?php 
													  		/*$sngCnt++;*/
								...
								<div class="tab-pane" id="driverTab" role="tabpanel">
									<div class="row">
											<div class="col-sm-12">
												  		...
												  		<tr>
															    	...
															    	<select name="driver_room_type" id="driver_room_type" class="form-control" data-msg-required="Please select type" required>
																		@if(!empty($cabinDetail))
											                               @foreach($cabinDetail as $k1=>$cabinval)
																			...
																			<option <?=(!empty($cart_experience->driver_room_type) && $cart_experience->driver_room_type == $cabinval->id)?'selected="selected"':''?> value="{{ $cabinval->id}}">{{$cabinval->name}}</option>
													      	...
													      	<td>
																...
																<div class="form_row mb-2 driver_text1">
															    	...
															    	@endhasrole
																		<input type="text" name="driver_name" id="driver_name" maxlength="100" class="driver_name form-control"  placeholder="" value="{{!empty($cart_experience->driver_name)?$cart_experience->driver_name:''}}"  /> 
																</div>
															
																<div class="form_row driver_text2">
																	
																	<input type="text" name="courier_name" id="courier_name" maxlength="100" class="courier_name form-control"  placeholder="" value="{{!empty($cart_experience->courier_name)?$cart_experience->courier_name:''}}" /> 
																</div>
													      	...
													      	<td class="">
													      		<div class="driver_check">
													      			<?php
										                                ...
										                                ?>
														      		<label class="checkbox_div <?php echo ($cart_experience->driver_paying == 1) ? 'selectedriver1' : ''; ?>">
											                          
											                          <input type="radio" class="custom_chkbox driver_paying" value="1" name="driver_paying" data-id="{{$cart_experience->id}}" <?php echo ($cart_experience->driver_paying == 1) ? 'checked' : 'checked'; ?> checked>  <span class="notClickedCls"></span>
											                          <span class="checkmark"></span>
											                          <span class="sk_label">Free</span>
											                        </label>

											                        <label class="checkbox_div <?php echo ($cart_experience->driver_paying == 2) ? 'selectedriver1' : ''; ?>">
											                          <input type="radio" class="custom_chkbox driver_paying " value="2" name="driver_paying" data-id="{{$cart_experience->id}}" <?php echo ($cart_experience->driver_paying == 2) ? 'checked' : ''; ?>> <span class="notClickedCls"></span>
											                          <span class="checkmark"></span>
											                          <span class="sk_label">Paying</span>
											                        </label>
											                        <input class="driver_cost" style="margin-left: 5px;margin-top: 5px;<?php echo ($cart_experience->driver_paying == 2) ? '' : 'display:none'; ?>"  placeholder="Enter Cost" value="{{!empty($cart_experience->driver_cost)?$cart_experience->driver_cost:''}}" type="digit" maxlength="5" pattern="[0-9]*" inputmode="numeric" name="driver_cost"/>
											                    <?php } else { ?>  <span class="sk_label"><?=($cart_experience->driver_paying == 2)?'Paying':'Free'?></span><?php } ?>
										                        </div>
										                        <div class="driver_check driver_text2" style="<?=(!empty($cart_experience->driver_room_type) && ($cart_experience->driver_room_type == '2' ||  $cart_experience->driver_room_type == '3'))?'display:block;':'display:block;'?> ">
										                        	<?php
										                                if (Auth::check() && !Auth::user()->hasRole("Collaborator")){
										                                ?>
														      		<label class="checkbox_div <?php echo ($cart_experience->driver_paying1 == 1) ? 'selectedriver1' : ''; ?>">
											                          <input type="radio" class="custom_chkbox driver_paying1" value="1" name="driver_paying1" data-id="{{$cart_experience->id}}" <?php echo ($cart_experience->driver_paying1 == 1) ? 'checked' : 'checked'; ?> > <span class="notClickedCls"></span>
											                          <span class="checkmark"></span>
											                          <span class="sk_label">Free</span>
											                        </label>

											                        <label class="checkbox_div <?php echo ($cart_experience->driver_paying1 == 2) ? 'selectedriver1' : ''; ?>">
											                          <input type="radio" class="custom_chkbox driver_paying1 " value="2" name="driver_paying1" data-id="{{$cart_experience->id}}" <?php echo ($cart_experience->driver_paying1 == 2) ? 'checked' : ''; ?>> <span class="notClickedCls"></span>
											                          <span class="checkmark"></span>
											                          <span class="sk_label">Paying</span>
											                        </label>
											                      	<input class="driver_cost1" style="margin-left: 5px;margin-top: 5px;<?php echo ($cart_experience->driver_paying1 == 2) ? '' : 'display:none'; ?>"  placeholder="Enter Cost" value="{{!empty($cart_experience->driver_cost1)?$cart_experience->driver_cost1:''}}" type="digit" maxlength="5" pattern="[0-9]*" inputmode="numeric" name="driver_cost1"/>
											                         <?php } else { ?>  <span class="sk_label"><?=($cart_experience->driver_paying1 == 2)?'Paying':'Free'?></span><?php } ?>
										                        </div>
													      	</td>
													      	<td>
																<input class="form-control"  placeholder="Enter Contact" value="{{!empty($cart_experience->driver_contact)?$cart_experience->driver_contact:''}}" type="digit" maxlength="12" pattern="[0-9]*" inputmode="numeric" name="driver_contact"/>
													      	</td>
													      	
													    </tr>
												  		<tr class="second_driver_div" <?=!empty($cart_experience->driver_name1)?'':'style="display:none"'?>>
													      	<td scope="row">
														      	<div class="form-group">
																    <label class="graycls">Cabin</label>
															    	<select name="driver_room_type1" id="driver_room_type1" class="form-control" data-msg-required="Please select type" required>
																		@if(!empty($cabinDetail))
											                               @foreach($cabinDetail as $k1=>$cabinval)
																			...
																			<option <?=(!empty($cart_experience->driver_room_type1) && $cart_experience->driver_room_type1 == $cabinval->id)?'selected="selected"':''?> value="{{ $cabinval->id}}">{{$cabinval->name}}</option>
													      	...
													      	<td>
																...
																<div class="form_row mb-2 driver_text_second">
																	...
																	<input type="text" name="driver_name1" id="driver_name1" maxlength="100" class="driver_name1 form-control"  placeholder="" value="{{!empty($cart_experience->driver_name1)?$cart_experience->driver_name1:''}}"  /> 
																</div>
															
																<div class="form_row driver_text_second2">
																	
																	<input type="text" name="courier_name1" id="courier_name1" maxlength="100" class="courier_name1 form-control"  placeholder="" value="{{!empty($cart_experience->courier_name1)?$cart_experience->courier_name1:''}}" /> 
																</div>
													      	...
													      	<td class="">
													      		<div class="driver_check">
													      			<?php
										                                if (Auth::check() && !Auth::user()->hasRole("Collaborator")){
										                                ?>
														      		<label class="checkbox_div <?php echo ($cart_experience->driver_paying_second == 1) ? 'selectedriver1' : ''; ?>">
											                          
											                          <input type="radio" class="custom_chkbox driver_paying_second" value="1" name="driver_paying_second" data-id="{{$cart_experience->id}}" <?php echo ($cart_experience->driver_paying_second == 1) ? 'checked' : 'checked'; ?> checked>  <span class="notClickedCls"></span>
											                          <span class="checkmark"></span>
											                          <span class="sk_label">Free</span>
											                        </label>

											                        <label class="checkbox_div <?php echo ($cart_experience->driver_paying_second == 2) ? 'selectedriver1' : ''; ?>">
											                          <input type="radio" class="custom_chkbox driver_paying_second" value="2" name="driver_paying_second" data-id="{{$cart_experience->id}}" <?php echo ($cart_experience->driver_paying_second == 2) ? 'checked' : ''; ?>> <span class="notClickedCls"></span>
											                          <span class="checkmark"></span>
											                          <span class="sk_label">Paying</span>
											                        </label>
											                          <input class="driver_second_cost" style="margin-left: 5px;margin-top: 5px; <?php echo ($cart_experience->driver_paying_second == 2) ? '' : 'display:none'; ?>"  placeholder="Enter Cost" value="{{!empty($cart_experience->driver_second_cost)?$cart_experience->driver_second_cost:''}}" type="digit" maxlength="5" pattern="[0-9]*" inputmode="numeric" name="driver_second_cost"/>
											                    <?php } else { ?>  <span class="sk_label"><?=($cart_experience->driver_paying_second == 2)?'Paying':'Free'?><?=($cart_experience->cost1 == 2)?'('.$cart_experience->cost1.')':''?></span><?php } ?>
										                        </div>
										                        <div class="driver_check driver_text_second2" style="<?=(!empty($cart_experience->driver_room_type1) && ($cart_experience->driver_room_type1 == '2' ||  $cart_experience->driver_room_type1 == '3'))?'display:block;':'display:block;'?> ">
										                        	<?php
										                                if (Auth::check() && !Auth::user()->hasRole("Collaborator")){
										                                ?>
														      		<label class="checkbox_div <?php echo ($cart_experience->driver_paying_second1 == 1) ? 'selectedriver1' : ''; ?>">
											                          <input type="radio" class="custom_chkbox driver_paying_second1" value="1" name="driver_paying_second1" data-id="{{$cart_experience->id}}" <?php echo ($cart_experience->driver_paying_second1 == 1) ? 'checked' : 'checked'; ?> > <span class="notClickedCls"></span>
											                          <span class="checkmark"></span>
											                          <span class="sk_label">Free</span>
											                        </label>

											                        <label class="checkbox_div <?php echo ($cart_experience->driver_paying_second1 == 2) ? 'selectedriver1' : ''; ?>">
											                          <input type="radio" class="custom_chkbox driver_paying_second1" value="2" name="driver_paying_second1" data-id="{{$cart_experience->id}}" <?php echo ($cart_experience->driver_paying_second1 == 2) ? 'checked' : ''; ?>> <span class="notClickedCls"></span>
											                          <span class="checkmark"></span>
											                          <span class="sk_label">Paying</span>
											                        </label>
											                         <input class="driver_second_cost1" style="margin-left: 5px;margin-top: 5px; <?php echo ($cart_experience->driver_paying_second1 == 2) ? '' : 'display:none'; ?>"  placeholder="Enter Cost" value="{{!empty($cart_experience->driver_second_cost1)?$cart_experience->driver_second_cost1:''}}" type="digit" maxlength="5" pattern="[0-9]*" inputmode="numeric" name="driver_second_cost1"/>
											                         <?php } else { ?>  <span class="sk_label"><?=($cart_experience->driver_paying_second1 == 2)?'Paying':'Free'?><?=($cart_experience->cost1 == 2)?'('.$cart_experience->cost1.')':''?></span><?php } ?>
										                        </div>
													      	</td>
													      	<td>
																<input class="form-control"  placeholder="Enter Contact" value="{{!empty($cart_experience->driver_contact1)?$cart_experience->driver_contact1:''}}" type="digit" maxlength="12" pattern="[0-9]*" inputmode="numeric" name="driver_contact1"/>
													      	</td>
												...
												@hasrole('Super Admin')
												<input type="button" name="add_driver" id="add_driver" class="cta btn btn-primary" value="Add Second Driver" <?=!empty($cart_experience->driver_name1)?'style="display:none"':''?> >
												@endhasrole
												<input type="button" name="remove_driver" id="remove_driver"  class="cta btn btn-primary"  <?=!empty($cart_experience->driver_name1)?'':'style="display:none"'?> value="Remove Second Driver" >
											</div> 
									</div>
								</div>
							...
							<div class="save__total" @hasrole('Collaborator') <?php echo (!empty($guestListCompleted) ? 'style="pointer-events:none;"' : ''); ?> @endhasrole>
								...
								<input type="hidden" name="cruise_guest_list" value="1" id="cruise_guest_list">
						            ...
						            @foreach($items as $item)
			                            ...
			                            @if(!empty($its->pivot))
			                              ...
			                              @if($its->id == 3 && $its->pivot->completed != 1 ) 
											    <a class="cta trackingCompleted" data-src="" data-id="{{!empty($cart_experience->id)?$cart_experience->id:''}}" data-step="{{ $its->id }}"  style="color: #ffffff;font-size: 14px;"> Complete Tracking 1</a>  
											    <input type="hidden" name="c_step" id="c_step" value="{{ $its->id }}">

			                              @elseif($its->id == 4 && $its->pivot->completed != 1 )
			                              		<a class="cta trackingCompleted" data-src="" data-id="{{!empty($cart_experience->id)?$cart_experience->id:''}}" data-step="{{ $its->id }}"  style="color: #ffffff;font-size: 14px;"> Complete Tracking 2</a>  
			                              		<input type="hidden" name="c_step" id="c_step" value="{{ $its->id }}">
			                              @elseif($its->id == 6 && $its->pivot->completed != 1 )  
			                              		<a class="cta trackingCompleted" data-src="" data-id="{{!empty($cart_experience->id)?$cart_experience->id:''}}" data-step="{{ $its->id }}"  style="color: #ffffff;font-size: 14px;"> Complete Tracking 3</a>  
			                              		<input type="hidden" name="c_step" id="c_step" value="{{ $its->id }}">
			                                @elseif($its->id == 7 && $its->pivot->completed != 1 )  
			                              		<a class="cta trackingCompleted" data-src="" data-id="{{!empty($cart_experience->id)?$cart_experience->id:''}}" data-step="{{ $its->id }}"  style="color: #ffffff;font-size: 14px;"> Complete Guest List</a>  
			                              		<input type="hidden" name="c_step" id="c_step" value="{{ $its->id }}">
										  @endif             
						       ...
						       <!--  <a href="#" class="cta total">
						        ...
						        <a class="cta total" data-fancybox data-type="ajax" data-src="" href="/total-supplements-charge/{{$cart_experience->id}}"  style="color: #ffffff;font-size: 14px;">Total Charges</a>
						    </div>	
						    <div class="">
	                ...
	                <div class="accommodation_summary">
	                    ...
	                    <div class="heading">
	                    	@hasrole('Super Admin')
	                    	<!-- <label class="checkbox_div">
	                          <input type="checkbox" class="custom_chkbox printAcco" value="1" name="display_on_print" data-id="{{$cart_experience->id}}" <?php echo ($cart_experience->display_on_print == 1) ? 'checked' : ''; ?>> <span class="notClickedCls"></span>
	                          <span class="checkmark"></span>
	                        ...
	                        <div class="total_cost" style="float: right;margin-top: unset;">
		                        	<?php $d_cnt = (!empty($cart_experience->driver_name) && ($cart_experience->driver_room_type == '2' || $cart_experience->driver_room_type == '3'))?'1':'0';
						    		$s_cnt = ($cart_experience->driver_room_type == '1')?'1':'0'; ?>
				                    ...
				                    @hasrole('Collaborator')
				                    	<?php //echo ($cart_experience->driver == 1) ? '- 1 - Dr' : ''; ?>
				                    	<?php //echo ($cart_experience->driver == 2) ? '- 2 - Dr & Cr' : ''; ?>
		                    ...
		                    <!-- <div style="margin-left: 20px;">
		                        ...
		                        <label class="checkbox_div <?php echo ($cart_experience->driver == 1) ? 'selectedriver' : ''; ?>">
		                          <input type="radio" class="custom_chkbox plusDriver" value="1" name="driver" data-id="{{$cart_experience->id}}" <?php echo ($cart_experience->driver == 1) ? 'checked' : 'checked'; ?>> <span class="notClickedCls"></span>
		                          <span class="checkmark"></span>
		                          <span class="sk_label">- 1 - Dr</span>
		                        </label>

		                        <label class="checkbox_div <?php echo ($cart_experience->driver == 2) ? 'selectedriver' : ''; ?>">
		                          <input type="radio" class="custom_chkbox plusDriver" value="2" name="driver" data-id="{{$cart_experience->id}}" <?php echo ($cart_experience->driver == 2) ? 'checked' : ''; ?>> <span class="notClickedCls"></span>
		                          <span class="checkmark"></span>
	                    ...
	                    <div class="accommodation_summary_table">
	                        ...
	                        <div class="accommodation_summary_row header">
	                            ...
	                            <div class="accommodation_summary_column">Cabin</div>
	                        </div>
	                        @if(!empty($cabinDetail))
                               @foreach($cabinDetail as $k1=> $cabinval)
                               @if(!empty($cabinCountAll[$cabinval->id]))
                               	<div class="accommodation_summary_row">
	                                <div class="accommodation_summary_column" style="padding-left: 40px;">{{$cabinCount[$cabinval->id]}}/{{$cabinCountAll[$cabinval->id]}}</div>
	                                <div class="accommodation_summary_column">{{$cabinval->name}}</div>
	                            </div>
	                            @endif
                               @endforeach
                            @endif
							 ...
							 <?php if(!empty($cart_experience->driver_room_type)){ ?>
							 	<div class="accommodation_summary_row">
	                                ...
	                                <div class="accommodation_summary_column"> <?php 
	                                    ...
	                                    $type1= $cart_experience->DriverRoomType1->driver_room_type;?>
	                                   Driver ({{$type1}})</div>
	                            </div>
	                        ...
	                        if(!empty($cart_experience->driver_room_type1) && $total_driver != '2' ){ ?>
	                        	<div class="accommodation_summary_row">
	                                ...
	                                <div class="accommodation_summary_column">
	                                	 <?php 
	                                        $room_type = array('1'=>'Single','2'=>'Double','3'=>'Twin');
	                                        $type2= $cart_experience->DriverRoomType2->driver_room_type1;?>
	                                       Driver ({{$type2}})
	                                </div>
	                            </div>
	                ...
	                @hasrole('Collaborator')
	                <?php
	                if(empty($cart_experience->single) || empty($cart_experience->double) || empty($cart_experience->twin)){ $display = 1;} ?>
	                @endhasrole
	                <div class="accommodation_change" <?=!empty($display)?'style="display:none;"':''?> >

	                	<input type="hidden" id="updated_held_last_date" name="updated_held_last_date" value="<?php echo !empty($cart_experience->held_last_date) ? date("Y-m-d",strtotime($cart_experience->held_last_date)) : '' ?>"> 
	                	<input type="hidden" id="updated_held_last_time" name="updated_held_last_time" value="<?php echo !empty($cart_experience->held_last_time) ? date("H:i",strtotime($cart_experience->held_last_time)) : '' ?>">
	                   <!--  <p>
	                        Please note that the following rooms are being held on option until <span class="held_last_datetime"><?php echo !empty($cart_experience->held_last_time) ? date("h:i A",strtotime($cart_experience->held_last_time)) : ''; ?> on <?php echo !empty($cart_experience->held_last_date) ? date("d F 'y",strtotime($cart_experience->held_last_date)) : ''; ?></span> after which any unsold rooms will be released back at no cancellation charge:
	                    </p>

	                    <p id="showRooms">
	                        <strong><?php echo !empty($cart_experience->single) ? $cart_experience->single : '0'; ?> Single, <?php echo !empty($cart_experience->double) ? $cart_experience->double : '0'; ?> Double room, <?php echo !empty($cart_experience->twin) ? $cart_experience->twin : '0'; ?> Twin room</strong>
	                    </p>
	                ...
	                <div class="download__print">
	                    ...
	                    <a data-fancybox="" data-type="ajax" data-src="" href="{{route('download-guest-list-multiple',$cart_experience->id)}}" class="cta">
					                               Download
					                            </a>
                		@else
                		<a target="_blank" href="{{route('download-guest-list-pdf',$cart_experience->id)}}" class="cta">
	                        Download
	                    </a>
                		@endif
	                    ...
	                    <?php  if (Auth::check() && Auth::user()->hasRole("Super Admin")){ ?>
	                    <a target="_blank" href="{{route('send-guest-list-pdf',$cart_experience->id)}}" class="cta">
	                         Send to Hotel
	                    </a>
	                   <?php } ?>
                ...
                <div class="bookingForm fancyboxTourSteps">
                    ...
                    <form method="POST" action="{{ route('process-tour-steps.ajax') }}" class="ajax">
                        ...
                        @elseif(optional($itemStatus)->id == 3 || optional($itemStatus)->id == 4 || optional($itemStatus)->id == 6 || optional($itemStatus)->id == 7)
				    		...
				    		$s_cnt = ($cart_experience->driver_room_type == '1')?'1':'0';
				    		$total_pax = get_total_pax($cart_experience->id); ?>
                            <!-- <div class="text-center tx-big">Pax: <strong><span class="t_pax">{{$total_pax}}</span><?php echo ($cart_experience->driver_name != '' && $cart_experience->driver_name1 != '') ? '+ 2 Dr' : (($cart_experience->driver_name != '') ? '+ 1 Dr' : ''); ?></strong></div>
							...
							<?php } ?>
                             ...
                             <button type="submit" data-step="{{ optional($itemStatus)->id }}" class="PaststepSubmitButtonTracking sk_cta">Save and update {{ optional($itemStatus)->name }}</button>
                         <?php /*}*/ ?>
...
<script>
	...
	$(document).on('click', '#printGuestlist', function(e) {
        ...
        $('.loader').show();    
        $.ajax({
            url: "{{ route('print-guest-list-pdf', $cart_experience->id) }}",
            type: 'POST',
	...
	$('#changeRooms').click(function() {
		...
		<?php if (Auth::check() && !Auth::user()->hasRole("Collaborator")){ ?>		    	
                   $('.held_last_datetime').html('<input type="date" min="<?=date('Y-m-d')?>" id="held_last_date" name="held_last_date" class="datepicker" value="<?php echo !empty($cart_experience->held_last_date) ? date("Y-m-d",strtotime($cart_experience->held_last_date)) : '' ?>"> &nbsp;<input type="time" id="held_last_time" name="held_last_time" value="<?php echo !empty($cart_experience->held_last_time) ? date("H:i",strtotime($cart_experience->held_last_time)) : '' ?>">');
                  /* $(".datepicker").datepicker({
				        endDate:'today'
				    });*/
                <?php }?>
	...
	$('#saveRooms').click(function() {
		...
		$.ajax({
		    url: "{{ route('extra-add-rooms', $cart_experience->id) }}",
		    type: 'POST',
		    dataType:'json',
...
$(".edit_room_req").bind('click', function(e){

	var id = $(this).data("id");
	$('.edit_req_div_'+id).show();
	$('.view_req_div_'+id).hide();
...
$(".save_room_req").bind('click', function(e){	
			...
			$('.loader').show();
		    $.ajax({
			    url: "{{ route('update-room-request-action-cruise') }}",
			    type: 'POST',
...
$(".save_upgrade").bind('click', function(e){	
	...
	$(".remove_upgrade_sup").bind('click', function(e){		
	if(confirm("Are you sure you want to remove?"))
	{
		var upgrade_id = $(this).data('id');
		var ele = $(this);
		$.ajax({
		    url: "{{ route('remove-upgrade-cruise') }}",
		    type: 'POST',
	...
	/*$('#secondary_list').click(function(){
		$('#primary_guest_list').removeClass('active');
		$(this).addClass('active');
		$('.table tr').hide();
		$('.table tr.paid_row').show();
	})*/
	$('.multiguestlist').click(function(){
		$('.loader').show();
		//$('.multiguestlist').removeClass('active');
		
		var exp_id = $(this).attr('data-exp-id');
		var exp_type = $(this).attr('data-exp-type');
		var hotel_name = $(this).attr('data-hotel-name');
		var hotel_id = $(this).attr('data-hotel-id');
		var cart_exp_id = '<?=$cart_exp_id?>';
		$('.expcls'+exp_id).addClass('active');
		if(exp_type == 1)
		{

			var page_url = "/change-rooms/"+cart_exp_id+"?exp_id="+exp_id+"&exp_type="+exp_type+"&hotel_id="+hotel_id+"&hotel_name="+hotel_name;
		}
		else
		{
			var page_url = "/editNumbersMultiguestlist/"+cart_exp_id+"?exp_id="+exp_id+"&exp_type="+exp_type+"&hotel_id="+hotel_id+"&hotel_name="+hotel_name;
		}
		 $.ajax({
			   url:page_url,
			   success:function(data)
			   {
			    $('.multiguestlist_html').html('');
			    $('.multiguestlist_html').html(data);
			    $('.loader').hide();
			   }
			  })
	})
	$('.unlink_row').click(function(){
		
		$(this).parent().parent('tr').removeClass('unselectableclick');
	})
</script>
</div>