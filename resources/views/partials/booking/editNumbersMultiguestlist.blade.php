				...
				<div class="grouped_header">
	                <div class="header">
	                    ...
	                    <div class="main_details">
	                        ...
	                        <div class="detail">
	                            Hotel: {{!empty($hotel_name)?$hotel_name:$cart_experience[0]->hotel_name}}
	                        </div>
...
DB::connection('mysql_veenus')->table('roomsmulti_supplements_requests')->where('cart_id', $cart_experience[0]->id)->where('upgrade_request_sup_status','pending')->groupBy('cart_experiences_to_rooms_id')->count();
	                        	?>
						...
						@else
							...
							@if ($sngLastAmt > 0 || $dblLastAmt > 0 || $twnLastAmt > 0 || $trpLastAmt > 0 || $qrdLastAmt > 0 )
						  	...
						  	@if ($sngLastAmt > 0)
								<li class="nav-item {{(!empty($LastAddedRequest) && $LastAddedRequest->room_id == 1)?'active':(empty($LastAddedRequest)?'active':'')}}" data-toggle="tab" href="javascript:;" data-href="#SignleRooms" role="tab">
									<?php if(!empty($sngSCntLinkstr1)){ ?>
									<span data-tooltip="<?php
										echo "This room is not linked. " ."\n"."Diffrent allocation on :"."\n".rtrim($sngSCntLinkstr1, ', '); ?>" data-tooltip-position="top"><i style="color:orange;" class="fa fa-unlink" aria-hidden="true"></i></span>
						  	...
						  	@if ($dblLastAmt > 0)
								...
								<li class="nav-item  {{(!empty($LastAddedRequest) && $LastAddedRequest->room_id == 2)?'active':''}}" data-toggle="tab" href="javascript:;" data-href="#DoubleRooms" role="tab">
									<?php if(!empty($dblSCntLinkstr1)){ ?>
									<span data-tooltip="<?php
										echo "This room is not linked. " ."\n"."Diffrent allocation on :"."\n".rtrim($dblSCntLinkstr1, ', '); ?>" data-tooltip-position="top"><i style="color:orange;" class="fa fa-unlink" aria-hidden="true"></i></span>
						  	...
						  	@if ($twnLastAmt > 0)
								<li class="nav-item {{(!empty($LastAddedRequest) && $LastAddedRequest->room_id == 3)?'active':''}}" data-toggle="tab" href="javascript:;" data-href="#TwinRooms" role="tab">
									<?php if(!empty($twnSCntLinkstr1)){ ?>
									<span data-tooltip="<?php
										echo "This room is not linked. " ."\n"."Diffrent allocation on :"."\n".rtrim($twnSCntLinkstr1, ', '); ?>" data-tooltip-position="top"><i style="color:orange;" class="fa fa-unlink" aria-hidden="true"></i></span>
							...
							@if ($trpLastAmt > 0)
								<li class="nav-item {{(!empty($LastAddedRequest) && $LastAddedRequest->room_id == 4)?'active':''}}" data-toggle="tab" href="javascript:;" data-href="#TripleRooms" role="tab">
									<?php if(!empty($trpSCntLinkstr1)){ ?>
									<span data-tooltip="<?php
										echo "This room is not linked. " ."\n"."Diffrent allocation on :"."\n".rtrim($trpSCntLinkstr1, ', '); ?>" data-tooltip-position="top"><i style="color:orange;" class="fa fa-unlink" aria-hidden="true"></i></span>
							...
							@if ($qrdLastAmt > 0)
								<li class="nav-item {{(!empty($LastAddedRequest) && $LastAddedRequest->room_id == 5)?'active':''}}" data-toggle="tab" href="javascript:;" data-href="#QuadrupleRooms" role="tab">
									<?php if(!empty($qrdSCntLinkstr1)){ ?>
									<span data-tooltip="<?php
										echo "This room is not linked. " ."\n"."Diffrent allocation on :"."\n".rtrim($qrdSCntLinkstr1, ', '); ?>" data-tooltip-position="top"><i style="color:red;" class="fa fa-exclamation-triangle" aria-hidden="true"></i></span>
...
DB::connection('mysql_veenus')->table('users')->where('id', $evalue->user_id)->first();
	            		...
	            		<form method="post" action="{{ route('colaborator-add-main-multirooms') }}" id="dataUpdateNumberHolder" class=" skalltabs sk_Rooms">
							<input type="hidden" value="{{$cart_experience[0]->id}}" id="current_cart_id" name="cart_id" class="cartIdSCls">
							<input type="hidden" value="{{$exp_id}}" id="exp_date_id" name="exp_date_id" class="exp_date_id">
							<input type="hidden" value="{{$multi_hotel_id}}" id="hotel_id" name="hotel_id" class="hotel_id">
							<input type="hidden" value="{{$experience_type}}" id="experience_type" name="experience_type" class="experience_type">
							@hasrole('Collaborator')
	                    						...
	                    						@else
												  ...
												  <tbody class="tblBorderCls">
												  	...
												  	foreach ($cartExperiencesToRooms as $key => $value) {
													  	...
													  	$paid_row = ($value->is_link == 1)?'unclick': '';
												  	 ?>
												  	 @hasrole('Collaborator')
												  	 <?php
														...
														?>
												  		@else
												  		<tr class="<?=($value->is_link == 1)?'unclick': ''?> table table-disaled">
											  			@endhasrole
													      	<td scope="row" class="unselectableclick" style="text-align:center;">
													      		<?php
															...
															?>
													      		...
													      		$supplements = array();
													      		$roomsSupplementsRequest = array();
													      		if(!empty($value->is_link))
													      		{

													      		 $roomsSupplementsRequest = App\Models\Cms\RoomsSupplementsRequest::where("cart_experiences_to_rooms_id", $value->primary_room_id)->orderBy('updated_at','desc')->get()->toArray();
													      		 $cart_experiences_to_rooms_id = $value->primary_room_id;
													      		}
													      		else
													      		{
													      			$roomsSupplementsRequest = App\Models\Cms\RoomsMultiSupplementsRequest::where("cart_experiences_to_rooms_id", $value->id)->orderBy('updated_at','desc')->get()->toArray();
													      			$cart_experiences_to_rooms_id = $value->id;
													      		}
													      		if(!empty($roomsSupplementsRequest)){
													      		...
													      		if(!empty($supplements)){
													      			...
													      			if(!empty($value->upgrade_request_status)){
															      			if(empty($pen_can)){ ?>
															      			<br><a data-fancybox data-type="ajax" data-src="" href="/request-multi-supplements/{{$value->id}}/{{$HotelDatesID}}/{{$cart_experience[0]->id}}"  style="color: #FCA311;font-size: 14px;">Add</a>
															      				<?php } } 
													      		}else{
													      		?>
													      		...
													      		<?php } }else{ ?>
													      			<br><a data-fancybox data-type="ajax" data-src="" href="/request-multi-supplements/{{$value->id}}/{{$HotelDatesID}}/{{$cart_experience[0]->id}}"  style="color: #FCA311;font-size: 14px;" class="up-request">Request</a>
													      		<?php } ?>
																 @else
																	<?php if(!empty($value->upgrade_request_status)){
																	if(!empty($pen_can)){  ?>
													      	...
													      	<td>
													      		...
													      		@else
													      		@if(!empty($value->is_link))
													      		<a class="unlink_row btn" data-id="{{$value->id}}"><i class="fa fa-link" aria-hidden="true"></i></a>
													      		@else
													      		<a class="link_row btn" style="" data-id="{{$value->id}}"><i class="fa fa-unlink" aria-hidden="true" style="color:orange;"></i></a>
													      		@endif
													      		<input type="hidden"  name="isLinkEditId[1][{{$value->id}}]" value="{{$value->primary_room_id}}">
													      		<input type="hidden" class="is_link_cls_{{$value->id}}" name="isLinkEdit[1][{{$value->id}}]" value="{{$value->is_link}}">
													      		<?php 
													      		if(count($experienceDates) > 1)
													      		{
														      		if(empty($value->primary_room_id)){?>
														      		<span data-tooltip="<?php
																				echo "This room is not linked with primary. "; ?>" data-tooltip-position="top"><i style="color:red;" class="fa fa-exclamation-triangle" aria-hidden="true"></i></span>
																<?php } }?>
												  	...
												  	for ($i=1; $i <= $sgl-$sngCnt+1; $i++) { ?>
													   <!--  <tr>
													      	<td scope="row" class="unselectableclick">
																...
																<td>
														      	...
														      	<td class="suppliment" >
																	...
																	<?php /*if(!empty($value->upgrade_request_status)){*/ ?>
														      		...
														      		<?php } }else{ ?>
														      			<br><a data-fancybox data-type="ajax" data-src="" href="/request-multi-supplements/{{$value->id}}/{{$HotelDatesID}}/{{$cart_experience[0]->id}}"  style="color: #FCA311;font-size: 14px;" class="up-request">Request</a><br>
														      		<?php } ?>
																	 @else
																		<?php if(!empty($value->upgrade_request_status)){
																		if(!empty($pen_can)){  ?>
											  			...
											  			@endhasrole
															...
															?>
													      	...
													      	<td>
													      		...
													      		<a class="link_row btn" style="" data-id="{{$value->id}}"><i class="fa fa-unlink" aria-hidden="true" style="color:orange;"></i></a>
													      		@endif
													      		<input type="hidden"  name="isLinkEditId[2][{{$value->id}}]" value="{{$value->primary_room_id}}">
													      		<input type="hidden" class="is_link_cls_{{$value->id}}" name="isLinkEdit[2][{{$value->id}}]" value="{{$value->is_link}}">
													      		<?php 
													      		if(count($experienceDates) > 1)
													      		{
														      		if(empty($value->primary_room_id)){?>
														...
														?>
											  			...
											  			@endhasrole
													      	...
													      	<td>
													      		...
													      		<a class="link_row btn" style="" data-id="{{$value->id}}"><i class="fa fa-unlink" aria-hidden="true" style="color:orange;"></i></a>
													      		@endif
													      		<input type="hidden"  name="isLinkEditId[3][{{$value->id}}]" value="{{$value->primary_room_id}}">
													      		<input type="hidden" class="is_link_cls_{{$value->id}}" name="isLinkEdit[3][{{$value->id}}]" value="{{$value->is_link}}">
													      		<?php 
													      		if(count($experienceDates) > 1)
													      		{
														      		if(empty($value->primary_room_id)){?>
														...
														?>
											  			...
											  			@endhasrole
													      	...
													      	<td>
													      		...
													      		<a class="link_row btn" style="" data-id="{{$value->id}}"><i class="fa fa-unlink" aria-hidden="true" style="color:orange;"></i></a>
													      		@endif
													      		<input type="hidden"  name="isLinkEditId[4][{{$value->id}}]" value="{{$value->primary_room_id}}">
													      		<input type="hidden" class="is_link_cls_{{$value->id}}" name="isLinkEdit[4][{{$value->id}}]" value="{{$value->is_link}}">
													      		<?php 
													      		if(count($experienceDates) > 1)
													      		{
														      		if(empty($value->primary_room_id)){?>
														...
														?>
											  			...
											  			@endhasrole
													      	...
													      	<td>
													      		...
													      		<a class="link_row btn" style="" data-id="{{$value->id}}"><i class="fa fa-unlink" aria-hidden="true" style="color:orange;"></i></a>
													      		@endif
													      		<input type="hidden"  name="isLinkEditId[5][{{$value->id}}]" value="{{$value->primary_room_id}}">
													      		<input type="hidden" class="is_link_cls_{{$value->id}}" name="isLinkEdit[5][{{$value->id}}]" value="{{$value->is_link}}">
													      		<?php 
													      		if(count($experienceDates) > 1)
													      		{
														      		if(empty($value->primary_room_id)){?>
...
$(".save_upgrade").bind('click', function(e){	
	...
	$(document).on("click", ".unlink_row", function() {
		var id = $(this).data('id');
	    $(this).parent().parent('tr').removeClass('unclick');
		$(this).html('<i class="fa fa-unlink" aria-hidden="true" style="color:orange;"></i>');
		$(this).removeClass('unlink_row');
		$(this).addClass('link_row');
		$('.is_link_cls_'+id).val('0');
	});
	$(document).on("click", ".link_row", function() {
		 var _this = this;
		$.confirm({'title': 'Re-link', 'message': " <h4 class='black'> Are you sure want to re-link that room?</h4>" + "<p>(Please note that this will remove any changes made and reconnect it with the primary guest list)</p>", 'buttons': {'Re-Link': {'class': 'btn btn-primary mr-1',
                      'action': function() {
                              var id = $(_this).data('id');
							    $(_this).parent().parent('tr').addClass('unclick');
								$(_this).html('<i class="fa fa-link" aria-hidden="true"></i>');
								$(_this).removeClass('link_row');
								$(_this).addClass('unlink_row');
								$('.is_link_cls_'+id).val('1');
                      }}, 'Close': {'class': 'btn btn-primary black-back'}}});
