                                ...
                                @elseif(optional($itemStatus)->id == 3 || optional($itemStatus)->id == 4 || optional($itemStatus)->id == 6)
                                    ...
                                    <div class="rooms" style="text-align:center;">
                                        <strong>{{$sngtotal}}</strong> sgl <strong>{{$dbltotal}}</strong> dbl <strong>{{$twntotal}}</strong> twn
                                            ...
                                            <input type="hidden" name="step{{$stepnumber}}" class="sk_small_text stepTracking{{$stepnumber}}" value="1">
                                            <input type="hidden" name="pax" class="" value="{{$rooms_ppl+$d_cnt}}">
...
$("body").on("click", '.showHideExep', function(e) {
    ...
    //calculate per person cost
     $("body").on("change", '.fancybox-container .calculate_cost', function(e) {  

    var transfer_cost = $('.fancybox-container #transfer_cost').val();
    var per_day_price = $('.fancybox-container #per_day_price').val();
    var added_margin = $('.fancybox-container .added_margin').val();
    ...
    if(total_amount >= '0')
    {
        $('.fancybox-container .total_cal_price').text('£'+total_amount);
        total_mar_amt = (total_amount*added_margin)/100;
        total_price = total_mar_amt+total_amount;
    }
    $('.fancybox-container .total_price').text('£'+total_price);
     ...
     $("body").on("click","ul.notestabs > li", function() {
        ...
        $(this).addClass('active');
         ...
         $("body").on("click", '.fancybox-container #add_notes_popup', function(e) {
        var cartExperienceId = $(this).data("id");
        var user_type = $(this).data("user_type");
        var category = $(this).data("cat");

        //$(".fancybox-container #fancybox_add_"+cartExperienceId+"_"+category).find('#user_type').val(user_type);
         $(".fancybox-container #fancybox_add_"+cartExperienceId+"_"+category).find('#category').val(category);
        $.fancybox.open(
        //$("#bookingFormBox-"+cartExperienceId).html() , {
        $(".fancybox-container #fancybox_add_"+cartExperienceId+"_"+category).html() , {
            closeExisting: false,
            touch: false,
            width:800,
            height:500,
         ...
         //$('.fancybox-container #ajax-file-upload').submit(function(e) {
        ...
        else
        {
            ...
            $.ajax({
                ...
                success:function(data)
                {
                    ...
                    $('.fancybox-container #tab_id_'+cart_id).html('');
                    $('.fancybox-container #tab_id_'+cart_id).html(data);


                   /* var initials = $('.fancybox-container #initials').val('');
                    var category = $('.fancybox-container #category').val('');
                    var note = $('.fancybox-container #notes').text('');
                    var cart_id = $('.fancybox-container #cart_id').val('');*/
                }
           });
        }
    ...
    $("body").on("click", '.fancybox-container .delete-note', function(e) {     
    ...
    $("body").on("click", '.fancybox-container #general_notes', function(e) {  
    ...
    $("body").on("click", '.fancybox-container #ame_notes', function(e) {  
    alert('Are you sure you want to delete this Note?');
    var cart_id =  $(this).data("cart_id");
    var ddata = [];
