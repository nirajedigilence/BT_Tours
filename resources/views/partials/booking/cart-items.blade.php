...
@if(isset($cart) && count($cart) > 0)
            ...
            <div class="section experienceItem" id="item-{{ $item['id'] }}" data-id="{{ $item['id'] }}">
                <div class="image">
                    @if(count($item['experience_images']) > 0)
                        <img src="{{ $image_url . 'storage/' . $item['experience_images'][0]['file'] }}"
                            alt="@if($item['experience_images'][0]['name']) {{ $item['experience_images'][0]['name'] }} @else {{ $item['experience_name'] }} @endif">
                    @endif
                </div>
                <div class="info">
                    <div class="heading">
                        <?php            $bt_user_data = getUserData();?>
                        {{ $item['experience_name'] }} <span class="removeButton"
                            style="float: right;font-size: 13px;text-transform: uppercase;"><a
                                href="{{ route('delete-cart-experience', [$item['id'], $bt_user_data['id']]) }}"
                                class="ajax removeLink" style="color: red;">Remove</a></span>
                    </div>
                    <div class="info_section">
                        <div class="info_table">
                            ...
                            <div class="info_row">
                                <div class="info_column">
                                    {{ date("D d M", strtotime($item["date_from"])) }} -
                                    {{ date("D d M 'y", strtotime($item["date_to"])) }} ( {{ $item["nights"] }}
                                    @if($item["nights"] > 1) nights @else night @endif )
                                </div>
                                <div class="info_column end">
    ...
    <form method="GET" action="{{ route('finalize-cart') }}" id="bookNowForm">
        ...
        <div class="footer1">
            ...
            <p>
                <input type="checkbox" value="1" name="term_condition" id="term_condition" required
                    style="height: 16px;width: 16px;"> Before completing your booking, please review our
                <!-- <a href="/terms" style="color:#FCA311;">terms and conditions</a>. --> <a class="add_booking"
                    id="t_link" style="color:#FCA311;" href="javascript:void(0)">terms and conditions</a>.
                <input type="hidden" name="created_by" value="{{ getBtUserValue() }}">
            </p>
    ...
    <div class="modal fade bd-example-modal-lg" id="termModel" tabindex="-1" role="dialog"
        aria-labelledby="addFolderModalTitle" aria-hidden="true" style="overflow-y: auto;">
                ...
                <div class="" style="margin-bottom: 200px;">
                    ...
                    <div class="container">
                        <div class="aboutContainer">
                            <a href="javascript:;" class="cta btn btn-primary" id="downloadPDFhc"
...
@else
    <p>
        <center>You have no experiences in the cart!</center>
    </p>
@endif