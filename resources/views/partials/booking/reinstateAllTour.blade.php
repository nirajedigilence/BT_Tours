       ...
       <!--  <div class="form-group">
     	...
     	<?php
            $cnt = 1;           
             ...
             if(!empty($valueEE->dates_rates_id))
             {
             	
             	if(!empty($valueEE->cart_exp_id) && empty($valueEE->unbooked) && $valueEE->cart_exp_id != $id)
             	{
             		continue;
             	}
...
DB::connection('mysql_veenus')->table('experience_dates')->selectRaw('experience_dates.id,experience_dates.experiences_id,experience_dates.unbooked,experience_dates.mark_as_completed,experience_dates.sign_name_hc,experience_dates.dates_rates_id,experience_dates.hotel_date_id,experience_dates.date_from,experience_dates.date_to,c.id as cart_exp_id,experience_dates.date_from,experience_dates.date_to,h.name as hotel_name,experience_dates.deleted_at')->leftjoin('cart_experiences as c','c.dates_rates_id','=','experience_dates.dates_rates_id')
        /* ->leftjoin('cart_experiences as c1','c1.old_date_rate_id','=','experience_dates.dates_rates_id')
         ...
         ->leftjoin('hotel_dates as hd','hd.id','=','experience_dates.hotel_date_id')->leftjoin('hotels as h','h.id','=','hd.hotel_id')->where('experience_dates.dates_rates_id',$valueEE->dates_rates_id)/*->where('experience_dates.deleted_at', null)*/->where('experience_dates.id','!=',$valueEE->id)/*->where('c.id', null)*/->groupBy('experience_dates.id')->get();
             }
            ...
            <div class="res_div">
              <p><b>{{ $valueEE->name }}</b></p>
		        <table id="myTable" class="table">
		                ...
		                <tbody>
		            <tr data-id="131" class="openBooking active" id="openBooking-131">
			            <td width="30%" class="col-name italic"><?php //pr($valueEE); ?>{{ !empty($valueEE->hotel_name)?$valueEE->hotel_name:$valueEE->cart_hotel_name }}</td>
			            ...
			            <td width="15%" class="col-cost visible italic">
			            	<?php $restore_yes = !empty($valueEE->cart_exp_id)?(empty($valueEE->restored_date)?'Yes':'Yes'):((!empty($valueEE->unbooked) && empty($valueEE->deleted_at))?'Yes':'No'); ?>{{$restore_yes}}</td>
			            <td width="25%" class="col-name visible">
			            	<?php if($restore_yes == 'Yes'){ ?> 
			            ...
			            <tr data-id="131" class="openBooking active" id="openBooking-131">
				            <td width="30%" class="col-name italic">{{ !empty($valueEEother->hotel_name)?$valueEEother->hotel_name:(!empty($valueEEother->cart_hotel_name)?$valueEEother->cart_hotel_name:'') }}</td>
				            ...
				            <td width="15%" class="col-cost visible italic">{{!empty($valueEEother->cart_exp_id)?(empty($valueEE->restored_date)?'Yes':'No'):((!empty($valueEE->unbooked) && empty($valueEE->deleted_at))?'Yes':'No')}}</td>
				            <td width="25%" class="col-name visible">
