...
<?php
   if($hotel_type == 2)
    {
        $overnight_data = $cart_experience->cartExperiencesToHotelCruiseOutbound;
    }
    else{
        $overnight_data = $cart_experience->cartExperiencesToHotelCruiseInbound;
    }
...
DB::connection('mysql_veenus')->table('carts')->selectRaw('carts.id,users.name,users.email')->join('users','carts.user_id','=','users.id')->where('carts.id', $cart_experience->carts_id)->first();
      if(isset($overnight_data))
        {
            $i=1;
         foreach ($overnight_data as $keyEE => $_valueEE) { 
        ...
        <div class="form-check">
			    		<div class="checkarea_part_Dates">
			                <label class="checkbox_div graycls" style="font-size: 13px;">
			                  <input type="checkbox" name="change_exp_new" class="custom_chkbox exp_checkbox tagchkbox notClickedCls "   value="{{$hotel_detail->id}}"> <?php echo (!empty($hotel_detail) ? $hotel_detail->name : ''); ?> (Overnight {{$i}})<span class="notClickedCls "></span>
...
DB::connection('mysql_veenus')->table('hotels')->selectRaw('id,name,address,contact_number,contact_name,phone,street,city,country,postcode')->where("id", $_valueEE->hotel_id)->first(); ?>
                <div class="changeExpNew_{{$hotel->id}} hotelDiv" style="display: none;">
					  <h3 id="tablist-1">Change Hotel -  <b><?php echo (!empty($hotel) ? $hotel->name : ''); ?> </b></h3>
					  <p>Select hotel to replace it with</p>
					       ...
					       <div class="form-group">
					       	<select class=" form-control expSelect_{{$hotel->id}} expChange hotelDropDown select2" name="step3[{{$hotel->id}}]"  required="" data-msg-required="Please select hotel" data-old-experience="{{$hotel->id}}">
...
<div class="screen_3" style="display:none;">
	...
	<p>&nbsp;</p>
	 <form method="post" enctype="multipart/form-data" action="{{ route('update-hotel-amend-cruise') }}" id="newAhemdDataHolder">      
                                {{ csrf_field() }}  
	  <?php
            if(isset($overnight_data))
            {
...
DB::connection('mysql_veenus')->table('hotels')->selectRaw('id,name,address,contact_number,contact_name,phone,street,city,country,postcode')->where("id", $_valueEE->hotel_id)->first(); ?>
                <div class="exp_div changeExpNew_{{$hotel->id}}" style="display: none;">
                            	...
                            	<div class="tc_box">
	                                ...
	                                <div class="body">
	                                  <!--  <label>Hotel name : <b><span class="exp_new"></span></b></label>
	                                   ...
	                                   <br> -->
	                                   <p>Hotel name :<?php echo (!empty($hotel) ? $hotel->name : ''); ?> </p>
					        ...
					        <div class="tc_box_wrapper">
	                            <div class="tc_box">
	                                ...
	                                <div class="body">
                                        ...
                                        <input type="hidden" name="hotel_new_{{$hotel->id}}" class="hotel_new" value="">
	                                   <p><span class="new_exp">Hotel name : <b><span class="exp_new"></span></b><span></p>
	                                   
	                                   <p>&nbsp;</p>
	                                   <p style="text-decoration: line-through !important; ">Hotel name : <b>><?php echo (!empty($hotel) ? $hotel->name : ''); ?>  </b></p>
        ...
        <?php $i++;} }  ?>
                ...
                <p style="">Ref No. '.$cart_experience->id.' - ('.$cart_experience->experience_name.'</p>';
                ...
                ';
								...
								<div style="clear:both;margin-left: 15px;"></div>
						        <input type="hidden" name="cart_exp_id" class="form-control"  value="{{!empty($cart_exp_id)?$cart_exp_id:''}}">
						        <input type="hidden" name="hotel_type" id="hotel_type" value="{{$hotel_type}}" class="btn btn-warning btn-sm ml-2">
						       <!--  <input type="submit" name="saveamount" value="Save" class="btn btn-warning btn-sm ml-2"> -->
...
<div class="screen_5" style="display:none;">
     <div class="changeExpNew">
                      ...
                      <p>Select hotel to add</p>
                            ...
                            <div class="form-group">
                            <select class=" form-control expSelect_ expChange hotelDropDown select2" name="hotel_name" id="hotel_name"  required="" data-msg-required="Please select hotel" data-old-experience="">
                                     <option value="">Select One</option>
                                        <?php 
                                            if(!empty($hotelList)){
                                            foreach ($hotelList as $kk => $val) {
                            ...
                            <div class="form-group">
                                        <label>Dates</label>
                                        <select class="form-control datesDropDown expDate_" name="hotel_date" id="hotel_date" required="">
                                            <option value="">-</option>
                                        </select>
...
<script type="text/javascript">
	...
	$( ".exp_checkbox" ).each(function( index ) {
		...
		{				
            ...
            $('.changeExpNew_'+value+' .hotel_new').val(selvalue);
        ...
        return false;
			...
			else
			{
				...
				//var params = $("#dataHolder").serialize();
				 let formData = new FormData(this);
		   		$.ajax({
				    url: "{{ route('update-hotel-amend-cruise') }}",
				    type: 'POST',
				    data: formData,
				    contentType: false,
		            processData: false,
