    ...
    <div class="accountContainer">
        ...
        <div class="accountRow">
            <div class="leftCol">
                ...
                @include('partials.collaborator_left',['open_menu' => 'bookings', 'sub_marked' => 'current_bookings']);
                @else
                @include('partials.super_user_left',['open_menu' => 'bookings', 'sub_marked' => 'current_bookings']);
                @endhasrole 
            </div>

            <div id="cartExperiencesPart">
                @hasrole('Collaborator')
                    @include ('partials.booking.collabrator_tour_overview_process_step', [
                                            'items' => $items,
                                            'tourStatuses' => $tourStatuses,
                                            'cart_exp_id' =>$cart_exp_id
                                          ])
                @else
                    @include ('partials.booking.tour_overview_process_step', [
                                            'items' => $items,
                                            'tourStatuses' => $tourStatuses,
                                            'cart_exp_id' =>$cart_exp_id
                                          ])
