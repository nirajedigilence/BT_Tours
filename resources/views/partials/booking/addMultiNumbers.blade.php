<div>
	...
	<h2>
		...
		{{$cart_experience[0]->hotel_name}} - Room requests
	</h2>
	<h5>
	...
	<br />
	<br />
	<h5 class="howmany_room">How many rooms do you need?</h5>
	...
	<br />
	<form method="post" action="{{ route('add-multi-rooms-detail') }}" id="dataHolder">
		...
		@for ($i = 1; $i <= 10; $i++)
		    <div class="roomHolder notReqCls" id="roomHolderNew{{$i}}">
				...
				?>
				<div class="form-group">
				<label class="graycls">&nbsp;</label>
				</div>
				<div class="form-group marginBottom0">
				    <label class="graycls"><h3><?php $tab = ($value['experience_type'] == 1)?'Primary':(($value['experience_type'] == 2)?'Secondary '.($secondary != '1'?$secondary:''):'Overnight '.$over_night );
						echo $tab;?> - <?=$value['hotel_name']?></h3></label>
		  		</div>
		  		<div class="form-group">
				    <label class="graycls">Room type</label>
				    <p>Please select a room type</p>
			    	<select name="room_type[{{$i}}][<?=$value['id']?>]" data-id="{{$value['id']}}" class="form-control room_type {{$i==1?'first_room':''}}" data-msg-required="Please select type" required>
						<option value="">Select</option>
						<?php foreach (allRoomType() as $key => $valuet) { ?>
								<option value="{{ $valuet->id }}">{{ $valuet->name }}</option>
						<?php } ?>
...
DB::connection('mysql_veenus')->table('hotel_dates_supplements')->where("hotel_dates_id", $value['hotel_date_id'])->get()->toArray();
		    	...
		    	$cls_dbl = '';
		    	$cls_dbl1 = '';
		    	foreach ($roomRequests as $key => $valuer) { 
		    		if(!empty($supplements) && in_array($valuer->id, array_column($supplements, 'supplements'))){
		    			$k = array_search($valuer->id, array_column($supplements, 'supplements'));
		    			...
		    			$sup = '';
		    				if($valuer->id == 1){
		    					...
		    					$cls_dbl = 'cls_dbl'.$value['id'];
		    					$cls_dbl1 = 'cls_dbl';
		    				}
		    		?>
			    	<div class="form-check sup_{{$valuer->id}}_{{$value['id']}}">
			    		<div class="checkarea_part_Dates">
			                <label class="checkbox_div graycls" style="font-size: 13px;">
			                  <input type="checkbox" name="room_requests[{{$i}}][<?=$value['id']?>][]" class="custom_chkbox tagchkbox notClickedCls {{$cls_dbl}} {{$cls_dbl1}}" data-id="{{$value['id']}}" value="{{ $valuer->id }}"> {{ $sup }} <?php if ( Auth::check() && Auth::user()->hasRole("Super Admin")){ ?><b> In </b>({{$currency_symbol}}{{$price_in[$k]}}) {{$price_type[$k]}}-  <b>Out</b> <?php } ?>({{$currency_symbol}}{{$price_out[$k]}}) - {{($price_type[$k] == 'pppn')?'pp':$price_type[$k]}}<span class="notClickedCls "></span>
				    ...
				    <div class="form-check" style="margin-left: 16px;">
			    		<div class="checkarea_part_Dates">
			                <label class="checkbox_div graycls">
			                  <input type="checkbox" name="room_requests_other[{{$i}}][<?=$value['id']?>][]" class="custom_chkbox tagchkbox notClickedCls other_supplement" value="5"> Other<span class="notClickedCls "></span>
			                  <span class="checkmark"></span>
			                </label>
			            </div	>
			        </div>
		        </div>
		        <div class="add_div_{{$value['id']}} other_div" style="display:none;">
		        	<button class="btn-sm btn-primary add_upgrade mt-5" type="button" data-value="{{$value['id']}}" id="add_upgrade">Add More</button>
					<div class="guest_list_upgrade_other">
						<!-- <input type="checkbox" id="other" /> <label for="other">Other:</label>  -->
							...
							<div class="guest_list_upgrade_other_control upgrade_name_div">
								...
								<input type="text" name="upgrade_name[{{$i}}][<?=$value['id']?>][]" maxlength="100" id="upgrade_name" class="upgrade-name" placeholder="Additional upgrade" />
							</div>
							
							<div class="guest_list_upgrade_other_control upgrade_cost_div">
								<label>Upgrade cost IN {{$currency_symbol}}</label>
								<input type="text" name="upgrade_cost[{{$i}}][<?=$value['id']?>][]" maxlength="5" id="upgrade_cost" class="upgrade-cost" / placeholder=" " />
							</div>
							<div class="guest_list_upgrade_other_control upgrade_cost_out_div">
								<label>Upgrade cost OUT {{$currency_symbol}}</label>
								<input type="text" name="upgrade_cost_out[{{$i}}][<?=$value['id']?>][]" maxlength="5" id="upgrade_cost_out" class="upgrade-cost-out" / placeholder=" " />
							</div>
							<div class="guest_list_upgrade_other_control upgrade_cost_type_div">
								<select class="form-control notClickedCls valid" name="upgrade_cost_type[{{$i}}][<?=$value['id']?>][]" class="upgrade_cost_type" fdprocessedid="24vei" aria-invalid="false">
				...
				<div class="guest_list_name guest_name_div_{{$value['id']}}">
		    		<div class="form-group marginBottom0">
			  		...
			  		<div class="appendguestname">
			  			<div class="form-group guestinput_{{$value['id']}} guest1_{{$value['id']}}" style="display: inline-block;width: 100%;">
						    <p>Guest Name 1</p>
				    		<input name="guest_name[{{$i}}][<?=$value['id']?>][]" maxlength="100" class="form-control"></input>
			  			</div>
			  			<div class="form-group guestinput_{{$value['id']}} guest2_{{$value['id']}}" style="display: inline-block;width: 100%;">
						    <p>Guest Name 2</p>
				    		<input name="guest_name[{{$i}}][<?=$value['id']?>][]" maxlength="100" class="form-control"></input>
			  			</div>
			  			<div class="form-group guestinput_{{$value['id']}} guest3_{{$value['id']}}" style="display: inline-block;width: 100%;">
						    <p>Guest Name 3</p>
				    		<input name="guest_name[{{$i}}][<?=$value['id']?>][]" maxlength="100" class="form-control"></input>
			  			</div>
			  			<div class="form-group guestinput_{{$value['id']}} guest4_{{$value['id']}}" style="display: inline-block;width: 100%;">
						    <p>Guest Name 4</p>
				    		<input name="guest_name[{{$i}}][<?=$value['id']?>][]" maxlength="100" class="form-control"></input>
			  			</div>
			  		</div>
		    	...
		    	<div class="form-group " style="display: inline-block;width: 100%;">
				     <p>Internal Note</p>
		    		<textarea name="room_note[{{$i}}]" maxlength="100" class="form-control"></textarea>
		  		</div>
		  		<?php } ?>
		  		<br>
...
<script>
	...
	$(document).on("change", ".room_type", function() {
		...
		if ($('.cls_dbl'+id).prop('checked')==true){ 
			var cls_dbl = $('.cls_dbl'+id).val();
		}
		else
		{
			var cls_dbl = '';
		...
		if(room_type == 1)
		{
			$(this).parent().parent().children('.sup_4_'+id).hide();
		}		
		else
		{
			$(this).parent().parent().children('.sup_4_'+id).show();
		}
	});
	...
	$("#addRooms").bind("click", function(e){
		...
		else
		{
			...
			var params = $("#dataHolder").serialize();
	   		$.ajax({
			    url: "{{ route('add-multi-rooms-detail') }}",
			    type: 'POST',
			    data: params,
			    success: function(data){
			        ...
			        toastSuccess('Room request has been added successfully.');
