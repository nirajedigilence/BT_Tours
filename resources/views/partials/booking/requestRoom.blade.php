<div>
	...
	<form method="post" action="{{ route('add-supplements-request') }}" id="dataHolderSup">
		...
		{{ csrf_field() }}
				...
				<div class="custom_adding_guest_list">
					...
					<h3>Upgrade <?=(!empty($cartexproom->room_id))?$room_type[$cartexproom->room_id]:''?> Room
		    		...
		    		<?php $price = array();
		    			...
		    			foreach ($hotelDatesSupplements as $key => $value) {
		    				...
		    				if($value['supplements'] == 1){
		    					...
		    					$sup = 'View (Garden/Balcony)';
		    					$price[2] = $value['price_out'];
		    					$price_in[2] = $value['price'];
		    					$price_type[2] = $value['price_type'];
		    				}elseif($value['supplements'] == 3){
		    					$sup = 'Executive/De Luxe/Superior Rooms';
		    					$price[3] = $value['price_out'];
		    					$price_in[3] = $value['price'];
		    					$price_type[3] = $value['price_type'];
		    				}elseif($value['supplements'] == 4){
		    					$sup = 'Dbl/tw room for sole';
		    					$price[4] = $value['price_out'];
		    					$price_in[4] = $value['price'];
		    					$price_type[4] = $value['price_type'];
		    				}
		    				}
		    			}
		    		...
		    		<!-- <div class="single_check">
							...
							@if(!empty($price_in[1]) || !empty($price[1]))
							<div class="single_check">
								<input type="checkbox" class="supchk" name="supplements[1][name]" id="sea" value="1" /> <label for="sea">Water view (Sea/Lake/River)  <?=!empty($price_in[1])?'<b>In :</b> '.$currency_symbol.$price_in[1].'':''?>   <?=!empty($price[1])?'<b>Out :</b> '.$currency_symbol.$price[1].'':''?> - <?=!empty($price_type[1])?($price_type[1] == 'pppn'?'pp':$price_type[1]).'':'pp'?> </label>
								<input type="hidden" value="{{!empty($price[1])?$price[1]:'0'}}" class="priceHidden" name="supplements[1][price]" value="">
							</div>
							@endif
							@if(!empty($price_in[2]) || !empty($price[2]))
							<div class="single_check">
								<input type="checkbox" class="supchk" name="supplements[2][name]" id="walk" value="2" /> <label for="walk">View (Garden/Balcony)  <?=!empty($price_in[2])?'<b>In :</b> '.$currency_symbol.$price_in[2].'':''?>   <?=!empty($price[2])?'<b>Out :</b> '.$currency_symbol.$price[2].'':''?> - <?=!empty($price_type[2])?($price_type[2] == 'pppn'?'pp':$price_type[2]).'':'pp'?> </label>
								<input type="hidden" value="{{!empty($price[2])?$price[2]:'0'}}" class="priceHidden" name="supplements[2][price]" value="">
							</div>
							@endif
							@if(!empty($price_in[3]) || !empty($price[3]))
							<div class="single_check">
								<input type="checkbox" class="supchk" name="supplements[3][name]" id="ground" value="3" /> <label for="ground">Executive/De Luxe/Superior Rooms <?=!empty($price_in[3])?'<b>In :</b> '.$currency_symbol.$price_in[3].'':''?>   <?=!empty($price[3])?'<b>Out :</b> '.$currency_symbol.$price[3].'':''?>  - <?=!empty($price_type[3])?($price_type[3] == 'pppn'?'pp':$price_type[3]).'':'pp'?> </label>
								<input type="hidden" value="{{!empty($price[3])?$price[3]:'0'}}" class="priceHidden" name="supplements[3][price]" value="">
							</div>
							@endif
							@if(!empty($price_in[4]) || !empty($price[4]))
							<div class="single_check">
								<input type="checkbox" class="supchk" name="supplements[4][name]" id="near" value="4" /> <label for="near">Dbl/tw room for sole <?=!empty($price_in[4])?'<b>In :</b> '.$currency_symbol.$price_in[4].'':''?>   <?=!empty($price[4])?'<b>Out :</b> '.$currency_symbol.$price[4].'':''?> - <?=!empty($price_type[4])?($price_type[4] == 'pppn'?'pp':$price_type[4]).'':'pp'?> </label>
								<input type="hidden" value="{{!empty($price[4])?$price[4]:'0'}}" class="priceHidden" name="supplements[4][price]" value="">
							</div>
							@endif
							<!-- <div class="single_check">
