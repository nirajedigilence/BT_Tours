...
DB::connection('mysql_veenus')->table('general_enquiries')->where('cart_experience_id', $cart_experience[0]->id)->where('is_resolved','0')->count();
                    ...
                    <div class="tour_sales">
                        ...
                        <div>Tour pax: {{$rooms_ppl+$d_cnt}}</div>
                    </div>

                </div>

                <div class="tabs_wrapper skalltabs sk_Rooms">
					@hasrole('Collaborator')
                	<ul class="nav tabs" role="tablist">
					  	@if ($sngLastAmt > 0)
					   	...
					   	@endif
					  	@if ($dblLastAmt > 0)
							<li class="nav-item" data-toggle="tab" href="javascript:;" data-href="#DoubleRooms" role="tab">Double Rooms ({{$dblSCnt}}/{{$dblLastAmt}})
							</li>
					   	@endif
					  	@if ($twnLastAmt > 0)
							<li class="nav-item" data-toggle="tab" href="javascript:;" data-href="#TwinRooms" role="tab">Twin Rooms ({{$twnSCnt}}/{{$twnLastAmt}})
							</li>
					   	@endif
						@if ($trpLastAmt > 0)
							<li class="nav-item" data-toggle="tab" href="javascript:;" data-href="#TripleRooms" role="tab">Triple Rooms ({{$trpSCnt}}/{{$trpLastAmt}})
							</li>
						@endif
						@if ($qrdLastAmt > 0)
							<li class="nav-item" data-toggle="tab" href="javascript:;" data-href="#QuadrupleRooms" role="tab">Quadruple Rooms ({{$qrdSCnt}}/{{$qrdLastAmt}})
							</li>
						@endif
						
						<li class="nav-item" data-toggle="tab" href="javascript:;" data-href="#driverTab" role="tab">Driver (<?php echo ($cart_experience[0]->driver_name != '' && $cart_experience[0]->driver_name1 != '') ? '2' : (($cart_experience[0]->driver_name != '') ? '1' : '0'); ?>/2)
						</li>
							...
							$s =1;
							  	foreach ($cartExperiencesToRoomsRequest as $key => $value) {
									  ...
									  case "1":
									    ...
									    break;
									  case "2":
									    $roomname = 'Double Rooms';
									    break;
									  case "3":
									    $roomname = 'Twin Rooms';
									    break;
									  case "4":
									    $roomname = 'Triple Rooms';
									    break;
									  case "5":
									    $roomname = 'Quadruple Rooms';
									    break;
									  default:
									    $roomname = "Single Room";
									}	
...
DB::connection('mysql_veenus')->table('hotel_dates_supplements')->where('hotel_dates_id', $hotel_dates->id)->where('supplements', $valueCRR['room_requests_id'])->first();
							  			 ...
							  			 {
							  			 	 $hotel_dates_supplements->price_type = !empty($hotel_dates_supplements->price_type)?$hotel_dates_supplements->price_type:'pppn';
                                                    $hotel_dates_supplements->price_type  = ($hotel_dates_supplements->price_type == 'pppn')?'pp':$hotel_dates_supplements->price_type;
                                                    $pstr = !empty($hotel_dates_supplements->price_out)?' (£'.$hotel_dates_supplements->price_out/*.' '.$hotel_dates_supplements->price_type*/.')':'';
							  			 }
							  			if($valueCRR['room_requests_id'] == 1){
						      						...
						      						$upgrade_name =  explode(',',$valueCRR['upgrade_name']);
							      						...
							      						if(!empty($upgrade_name))
							      						{
							      							...
							      							{
							      								$ct = !empty($upgrade_cost[$key])?' - In (£'.$upgrade_cost[$key].')':'';
							      								$ct1 = !empty($upgrade_cost_out[$key])?' Out (£'.$upgrade_cost_out[$key].')':'';
										    	...
										    	<div class="form-group marginBottom0">
											  	...
											  	<div class="sk_actions<?php echo $rowId; ?> initialdiv">    
												  	<?php 	if(empty($cnts))
													  	...
													  	<?php } ?>
													  	<div class="form-group ">
												  			 <label class="graycls">Special Requests</label>
												  	...
												  	<?php } else {?>
												  		<div class="form-group">
														    <label class="graycls">Additional Comments</label>
													    	<textarea readonly name="room_add_comment" class="form-control initialcls room_add_comment_<?php echo $rowId; ?>" >{{!empty($value->addtional_comment)?$value->addtional_comment:''}}</textarea>

													    </div>
													    <div class="form-group">
														    <label class="graycls">Guest Requests (max 100 characters)</label>
												...
												if($rrvalue['supplements'] == 1){
						      						...
						      						$upgrade_name =  explode(',',$rrvalue['upgrade_name']);
							      						...
							      						if(!empty($upgrade_name))
							      						{
							      							...
							      							foreach($upgrade_name as $key=>$row)
							      							{
							      								$str_add .= $row.' - In ('.$upgrade_cost[$key].')'.'- Out ('.$upgrade_cost_out[$key].')'.' - '.$upgrade_cost_type[$key].''.'';
							      							}
							      						}
						...
						if($cartExperiencesToRoomsCancellation->count()>0){
							...
							$s =1;
							  	foreach ($cartExperiencesToRoomsCancellation as $key => $value) {
								  	...
								  	$rowId = $value->id;
								  	if($value->cancellation_request_status == 'declined'){
								  		continue;
								  	}
							  	 ?>	
											  	...
											  	<div class="sk_actions<?php echo $rowId; ?> initialdiv">    
												  	<?php if($value->cancellation_request_status == '' || $value->cancellation_request_status == null){ ?>
												    	...
												    	<div class="form-group tour-sk">
														    <label class="graycls">Add a tour note</label>
												    		<textarea name="room_note" class="form-control croom_note_<?php echo $rowId; ?>" maxlength="255" rows="3"></textarea>
												  		</div>
												  		<div class="ctas sk_ctas">
...
DB::connection('mysql_veenus')->table('users')->where('id', $evalue->user_id)->first();
            		...
            		<form method="post" action="{{ route('colaborator-add-main-rooms') }}" id="dataUpdateNumberHolder" class=" skalltabs sk_Rooms">
                    						...
                    						@else
											  ...
											  <thead>
											    <tr>
											      ...
											      <th scope="col">@hasrole('Collaborator')Upgrade requests @else Room requests @endhasrole </th>
											      <th scope="col">@hasrole('Collaborator')Guest requests @else Guest requests @endhasrole </th> 
											      <th scope="col">&nbsp;</th>
											    </tr>
											  </thead>
											  <tbody class="tblBorderCls">
													...
													?>
										  			...
										  			@endhasrole
														...
														if($today >= $minusDaysUp){
															echo '<td class="sadasd table-disaled" style="pointer-events: none;">';
														}else{
														...
														?>
												      	...
												      	<td>
															<textarea class="form-control"  placeholder="Enter Guest requests" name="specialRequestsEdit[1][{{$value->id}}]">{{$value->specials}}</textarea>
												      	</td>
												      	<td>
												      		@hasrole('Collaborator')
											  	...
											  	for ($i=1; $i <= $sgl-$sngCnt+1; $i++) { ?>
												   <!--  <tr>
												      	...
												      	<td>
															<input type="text" class="form-control" placeholder="Enter Guest requests" name="specialRequests[1][{{$i}}]">
												      	</td>
                    						...
                    						@else
											  ...
											  <thead>
											    <tr>
											      ...
											      <th scope="col">Name</th>
											      <th scope="col">Upgrade requests </th>
											      <th scope="col">Guest requests</th>
											      <th scope="col">&nbsp;</th>
											    </tr>
											  </thead>
											  <tbody class="tblBorderCls">
													...
													?>
										  			...
										  			@endhasrole
												      			...
												      			foreach ($roomsSupplementsRequest as $rskey => $rsvalue) {
												      				...
												      				if($rsvalue['cart_experiences_to_rooms_id'] == $value->id){
												      					...
												      					if($rsvalue['supplements'] == 1){
														      						...
														      						$dbl_sole  = ($rsvalue['upgrade_request_sup_status'] == 'accepted'?'1':'1');
														      						$rsvalue['supplements'] = 'Dbl/tw room for sole : '.$rsvalue['upgrade_request_sup_status'];
														      					}elseif($rsvalue['supplements'] == 7){
														      						$rsvalue['supplements'] = 'Walk In Shower : '.$rsvalue['upgrade_request_sup_status'];
														      					}elseif($rsvalue['supplements'] == 8){
														...
														?>
												      	...
												      	<td>
															<textarea class="form-control"  placeholder="Enter Guest requests" name="specialRequestsEdit[2][{{$value->id}}]">{{$value->specials}}</textarea>
												      	</td>
												      	<td>
												      		@hasrole('Collaborator')
											  	...
											  	for ($i=1; $i <= $dbl-$dblCnt+1; $i++) { ?>
												   <!--  <tr>
												      	...
												      	<td>
															<input type="text" class="form-control" placeholder="Enter Guest requests" name="specialRequests[2][{{$i}}]">
												      	</td>
												      	<td>&nbsp;</td>
												    </tr> -->
											  	<?php } ?>
											  	 ...
											  	 <?php
													...
													?>
										  			...
										  			@endhasrole
												      	...
												      	<td>
															<textarea class="form-control"  placeholder="Enter Guest requests" name="specialRequestsEdit[3][{{$value->id}}]">{{$value->specials}}</textarea>
												      	</td>
												      	<td>
												      		@hasrole('Collaborator')
											  	...
											  	<?php 
												  	...
												  	for ($i=1; $i <= $twn-$twnCnt+1; $i++) { ?>
													    <!-- <tr>
												      	...
												      	<td>
															<input type="text" class="form-control" placeholder="Enter Guest requests" name="specialRequests[3][{{$i}}]">
												      	</td>
												      	<td>&nbsp;</td>
												    </tr> -->
												  	<?php } ?>
											  	 ...
											  	 ?>
											  		<tr>
									                    ...
									                    <?php } ?>
												      	...
												      	<td>
															<textarea class="form-control"  placeholder="Enter Guest requests" name="specialRequestsEdit[4][{{$value->id}}]">{{$value->specials}}</textarea>
												      	</td>
												      	<td>
												      		@hasrole('Collaborator')
											  	...
											  	<?php 
												  	...
												  	for ($i=1; $i <= $trp-$trpCnt+1; $i++) { ?>
													   <!--  <tr>
													      	...
													      	<td>
																<input type="text" class="form-control" placeholder="Enter Guest requests" name="specialRequests[4][{{$i}}]">
													      	</td>
													      	<td>&nbsp;</td>
													    </tr> -->
												  	<?php } ?>
											  	...
											  	foreach ($cartExperiencesToRooms as $key => $value) {
											  	 ...
											  	 ?>
											  		<tr>
												      	...
												      	<td>
															<textarea class="form-control"  placeholder="Enter Guest requests" name="specialRequestsEdit[5][{{$value->id}}]">{{$value->specials}}</textarea>
												      	</td>
												      	<td>
												      		@hasrole('Collaborator')
											  	...
											  	<?php 
												  	...
												  	for ($i=1; $i <= $qrd; $i++) { ?>
													   <!--  <tr>
													      	...
													      	<td>
																<input type="text" class="form-control" placeholder="Enter Guest requests" name="specialRequests[5][{{$i}}]">
													      	</td>
													      	<td>&nbsp;</td>
													    </tr> -->
												  	<?php } ?>
											  		...
											  		<tr>
												      	...
												      	<td class="">
												      		<div class="driver_check">
										                          ...
										                          <input type="radio" class="custom_chkbox " value="1" name="driver_paying" data-id="{{$cart_experience[0]->id}}" <?php echo ($cart_experience[0]->driver_paying == 1) ? 'checked' : 'checked'; ?> checked>  <span class="notClickedCls"></span>
										                          <span class="checkmark"></span>
									                        ...
									                        <div class="driver_check driver_text2" style="<?=(!empty($cart_experience[0]->driver_room_type) && ($cart_experience[0]->driver_room_type == '2' ||  $cart_experience[0]->driver_room_type == '3'))?'display:block;':'display:none;'?> ">
									                        	<?php
										                          ...
										                          <input type="radio" class="custom_chkbox " value="1" name="driver_paying1" data-id="{{$cart_experience[0]->id}}" <?php echo ($cart_experience[0]->driver_paying1 == 1) ? 'checked' : 'checked'; ?> > <span class="notClickedCls"></span>
										                          <span class="checkmark"></span>
										                          <span class="sk_label">Free</span>
										                        </label>
											  		...
											  		<tr class="second_driver_div" <?=!empty($cart_experience[0]->driver_name1)?'':'style="display:none"'?>>
												      	...
												      	<td class="">
												      		<div class="driver_check">
										                          ...
										                          <input type="radio" class="custom_chkbox " value="1" name="driver_paying_second" data-id="{{$cart_experience[0]->id}}" <?php echo ($cart_experience[0]->driver_paying_second == 1) ? 'checked' : 'checked'; ?> checked>  <span class="notClickedCls"></span>
										                          <span class="checkmark"></span>
										                          <span class="sk_label">Free</span>
										                        </label>

										                        <label class="checkbox_div <?php echo ($cart_experience[0]->driver_paying_second == 2) ? 'selectedriver1' : ''; ?>">
										                          <input type="radio" class="custom_chkbox " value="2" name="driver_paying_second" data-id="{{$cart_experience[0]->id}}" <?php echo ($cart_experience[0]->driver_paying_second == 2) ? 'checked' : ''; ?>> <span class="notClickedCls"></span>
										                          <span class="checkmark"></span>
										                          <span class="sk_label">Paying</span>
										                        </label>
									                        ...
									                        <div class="driver_check driver_text_second2" style="<?=(!empty($cart_experience[0]->driver_room_type) && ($cart_experience[0]->driver_room_type == '2' ||  $cart_experience[0]->driver_room_type == '3'))?'display:block;':'display:none;'?> ">
									                        	<?php
									                                if (Auth::check() && !Auth::user()->hasRole("Collaborator")){
									                                ?>
													      		<label class="checkbox_div <?php echo ($cart_experience[0]->driver_paying_second1 == 1) ? 'selectedriver1' : ''; ?>">
										                          <input type="radio" class="custom_chkbox " value="1" name="driver_paying_second1" data-id="{{$cart_experience[0]->id}}" <?php echo ($cart_experience[0]->driver_paying_second1 == 1) ? 'checked' : 'checked'; ?> > <span class="notClickedCls"></span>
										                          <span class="checkmark"></span>
										                          <span class="sk_label">Free</span>
										                        </label>

										                        <label class="checkbox_div <?php echo ($cart_experience[0]->driver_paying_second1 == 2) ? 'selectedriver1' : ''; ?>">
										                          <input type="radio" class="custom_chkbox " value="2" name="driver_paying_second1" data-id="{{$cart_experience[0]->id}}" <?php echo ($cart_experience[0]->driver_paying_second1 == 2) ? 'checked' : ''; ?>> <span class="notClickedCls"></span>
										                          <span class="checkmark"></span>
										                          <span class="sk_label">Paying</span>
										                        </label>
										                         <?php } else { ?>  <span class="sk_label"><?=($cart_experience[0]->driver_paying_second == 2)?'Paying':'Free'?><?=($cart_experience[0]->cost1 == 2)?'('.$cart_experience[0]->cost1.')':''?></span><?php } ?>
				    	...
				    	<?php $d_cnt = (!empty($cart_experience[0]->driver_name) && ($cart_experience[0]->driver_room_type == '2' || $cart_experience[0]->driver_room_type == '3'))?'1':'0';
				    		$s_cnt = ($cart_experience[0]->driver_room_type == '1')?'1':'0'; ?>
		                    Total PAX: {{$rooms_ppl+$d_cnt}} <!--$rooms_ppl +$d_cnt -->
                ...
                <div class="accommodation_change">
                ...
                <div class="bookingForm fancyboxTourSteps">
                    ...
                    <form method="POST" action="{{ route('process-tour-steps.ajax') }}" class="ajax">
                        ...
                        @elseif(optional($itemStatus)->id == 3 || optional($itemStatus)->id == 4 || optional($itemStatus)->id == 6 || optional($itemStatus)->id == 7)
                            ...
                            <?php $d_cnt = (!empty($cart_experience[0]->driver_name) && ($cart_experience[0]->driver_room_type == '2' || $cart_experience[0]->driver_room_type == '3'))?'1':'0';
				    		$s_cnt = ($cart_experience[0]->driver_room_type == '1')?'1':'0'; ?>
                            <div class="text-center tx-big">Pax: <strong><span class="t_pax">{{$rooms_ppl+$d_cnt}}</span><?php echo ($cart_experience[0]->driver_name != '' && $cart_experience[0]->driver_name1 != '') ? '+ 2 Dr' : (($cart_experience[0]->driver_name != '') ? '+ 1 Dr' : ''); ?></strong></div>
                            ...
                            <div class="rooms tx-big" style="text-align:center;">
                                <strong><span class="s_total">{{$sngtotal}}</span></strong> sgl <strong><span class="d_total">{{$dbltotal}}</span></strong> dbl <strong><span class="t_total">{{$twntotal}}</span></strong> twn
									...
									<input type="hidden" name="pax" class="tpax_total" value="{{$rooms_ppl+$d_cnt}}">
