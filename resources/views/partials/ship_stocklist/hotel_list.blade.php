            ...
            <div class="filters">
                ...
                <div class="filters_dropdown">
                    ...
                    <div class="dropdown">
                        ...
                        <form action="{{ route('stocklist-hotel') }}" id="filterForm">
                            ...
                            <div class="dropdown_section">
                                ...
                                <div class="by_date">
                                    ...
                                    <div class="field">
                                        <div class="label">By Location</div>
                                        <select name="country_areas_id" id="location">
                                            <option value=""></option>
                                            @foreach ($countries as $k => $country)
                                                <optgroup label="{{$country->name}}" data-max-options="1">
                                                    @foreach ($country->countryAreas as $kk => $countryArea)
                                                        <option value="{{ $countryArea->id }}" <?php echo (isset($_GET['country_areas_id']) && ($_GET['country_areas_id'] == $countryArea->id) ? 'selected' : ''); ?>>
                                                            {{ $countryArea->name }}
                                                        </option>
                                                    @endforeach
                                                </optgroup>
                                    ...
                                    <div class="field">
                                        <div class="label">Hotel - town/city</div>
                ...
                <ul class="active_filters">
                    <li class="label">Active filters:<br/>
                        <a href="{{ route('stocklist-hotel') }}" style="color: orange;font-weight: normal;text-decoration: underline !important;float: left;">Reset</a></li>
                    <?php if(isset($_GET['country_areas_id']) && $_GET['country_areas_id'] != ''){ ?>
                        <li>Location. <a href="javascript:;" id="locationnoRemove"><span>x</span></a></li>
                    <?php } ?>
            ...
            ?>
                <div class="left_column__list_row openBooking" data-id="{{ $proExValue->id }}" id="openBooking-{{ $proExValue->id }}">
                    ...
                    <div class="left_column__list_column w_20">
                        <?php
                        if(!empty($months)){
                            echo '<a href="javascript:;" class="viewAndAddDatesBtn hotel_listdate_'.$proExValue->id.'" data-id="'.$proExValue->id.'" style="color:orange;">'.implode(', ',(array_unique($months))).'</a>';
                        }else{
                            echo '-';
                        }
                        ?>
...
<div class="modal fade" id="hotelCancelDatesNoteModal" tabindex="-1" role="dialog" aria-labelledby="addFolderModalTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document" style="max-width: calc(50% - (1.75rem * 8));">
        <div class="modal-content">
            <div class="modal-body" style="padding: 30px;">
                <form method="POST" action="{{ route('stocklist-hotel-dates-remove') }}" method="post" enctype="multipart/form-data">
                    @csrf
                <h5 style="font-weight: bold;">Cancel Dates</h5>
