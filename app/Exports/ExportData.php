...
use DB;
class ExportData implements FromView,WithColumnFormatting
{
    protected $id;
    ...
    public function view(): View
    {
           /* $month = $this->month;
            ...
            $items = Cart::getExportData($filter_data["sort_by"],$filter_data["sort_type"]);
            if($_GET['excel_format'] == 2)
            {
                return view('partials.booking.export.experience_booking', [            
                    'items'=>$items                
                ]);
            }
            else
            {
                return view('partials.booking.export.export_excel_report', [            
                    'items'=>$items                
                ]);
            }
