...
{   
    ...
    {
         $this->middleware('auth.basic')->only('index');
    ...
    public function update_experience_data()
    {
   ...
   public function update_bt_settings()
   {


/*$client = new Client();
 try {
$response = $client->post('https://plus.curenearme.in/webhook', [
    'headers' => [
    ...
    'verify' => false, // disables SSL cert check - use only for testing
]);
...
exit;*/
   ...
   public function create_slug()
        {
           $experience = DB::table('experiences')->select('experiences.id','experiences.name')->get();
                $slugar = array();
              if(!empty($experience[0]))
              {
                foreach($experience as $item)
                {
                     ...
                     /*if(DB::table('experiences')->where('slug_name', $slug)->exists())
                     {
                        do {
                            $slug = Str::slug($item->name).'-'.rand(1,10);
                        } while (DB::table('experiences')->where('slug_name', $slug)->exists());
                     }
                     else
                     {
                        $slug = Str::slug($item->name);
                     }*/
                     if(in_array($slug,$slugar))
                     {
                        $slug = Str::slug($item->name).'-'.rand(1,10);
                     }
                     else
                  ...
                  $cartAttraction['slug_name']= $slug;
                
                  pr($cartAttraction);
                  //DB::table('experiences')->where('id',$item->id)->update($cartAttraction);
