    ...
    public function handle($request, Closure $next)
    {
        ...
        if($compileLess){
            try{
                ...
                $bespoke_lessFile = resource_path()."/less/style-bespoke.less";
                $bespoke_cssFile = public_path()."/css/style-bespoke.css";
                ...
                $parser = new \Less_Parser($options);
                $appUrl = env('APP_URL', 'http://localhost');
                $parser->parseFile( $bespoke_lessFile, $appUrl);
                $css = $parser->getCss();
                file_put_contents($bespoke_cssFile,$css);
