...
$(document).on('click', '.contractSelectCls', function() {
    if ($(".isSelectCls")[0]){
        ...
        var url= base_url+'/super-user/stocklist-edit-dates/'+hotel_date_id+'/'+hotel_id+'/'+'?contracts=1&new_date=1&pdf=1';
        window.open(url,'_blank')
        $('.isSelectCls').removeClass('isSelectCls');
    } else {
        //toastError('Please select atleast one dates');
         var hotel_id = $(this).attr("data-hotel-id");
         var url= base_url+'/super-user/stocklist-edit-dates/0/'+hotel_id+'/'+'?contracts=1&new_date=1&pdf=1';
        window.open(url,'_blank')
    }
});
$(document).on('click', '.addSelectCls', function() {
    if ($(".isSelectCls")[0]){
        var hotel_date_id = $(".isSelectCls").attr("data-date-id");
        var hotel_id = $(".isSelectCls").attr("data-hotel-id");
        var url= base_url+'/super-user/stocklist-edit-dates/'+hotel_date_id+'/'+hotel_id+'/'+'?contracts=1&new_date=1';
        window.open(url,'_blank')
        $('.isSelectCls').removeClass('isSelectCls');
    } else {
         var hotel_id = $(this).attr("data-hotel-id");
         var url= base_url+'/super-user/stocklist-edit-dates/0/'+hotel_id+'/'+'?contracts=1&new_date=1';
        window.open(url,'_blank')
        //toastError('Please select atleast one dates');
    }
});
...
function open_dates_rates_cancelled(hotelId,databutton)
{
    $('.loader').show();    
    $.ajax({
        url: base_url+'/super-user/stocklist-hotel-cancelled-dates',
        type: 'POST',
         // dataType: 'json',
        data: {'hotel_id':hotelId,'databutton':databutton},
        success: function(data) {
